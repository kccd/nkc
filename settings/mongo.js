...
const { username, password, database, address, port, poolSize, keepAlive } =
  mongodbConfig;
let account = '';
...
module.exports = () => {
  return new Promise((resolve, reject) => {
    const url =
      mongodbConfig.url || `mongodb://${account}${address}:${port}/${database}`;
    mongoose
      .connect(encodeMongoURI(url), options)
      .then(() => {
        resolve(mongoose);
      })
      .catch((e) => {
