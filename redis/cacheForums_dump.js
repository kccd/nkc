...
const cacheForums = async () => {
  ...
  * */
  ...
  for (const forum of forumsTree) {
    ...
    const getAccessibleForumsId = (forum) => {
      ...
      if(relation === 'or') {
        ...
        if(gradesId.length === 0 && rolesId.length === 0) return;
      ...
      for(const f of forum.childForums){
        ...
        const childAllowedGrades = [];
        const childAllowedRoles = [];

        for(let i = 0; i < gradesId.length; i++) {
          if(allowedGrades.includes(gradesId[i])) {
            childAllowedGrades.push(gradesId[i]);
          }
        }
        for(let i = 0; i < rolesId.length; i++) {
          if(allowedRoles.includes(rolesId[i])) {
            childAllowedRoles.push(rolesId[i]);
          }
        }
        if(f.fid === '175') {
          console.log(f.relation);
          console.log(childAllowedRoles);
          console.log(childAllowedGrades);
        }
        f.gradesId = childAllowedGrades;
        f.rolesId = childAllowedRoles;
  ...
  let key = `canNotDisplayInParentForumsId`;
