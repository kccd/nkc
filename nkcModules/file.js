...
async function getStoreUrl(t) {
  ...
  if (!storeUrl) {
    throwErr`未找到与时间匹配的存储服务，请检查文件 attachment.json 中的配置是否正确`;
  }
  return storeUrl;
}
...
async function getFileExtension(file, extensions = []) {
  ...
  if (extension !== pathExtension) {
    ...
    if (!jpgArr.includes(extension) || !jpgArr.includes(pathExtension)) {
      throwErr(
        400,
        `文件内容格式(${extension})与后缀名(${pathExtension})无法对应`,
      );
    }
  }
  if (!extension) {
    throwErr(400, '未知的文件格式');
  }
  if (extensions.length) {
    if (!extensions.includes(extension)) {
      throwErr(
        400,
        `文件格式不被允许，请选择${extensions.join(', ')}格式的文件。`,
      );
    }
  }
  return extension;
...
function filterFilesInfo(files) {
  ...
  for (const type in files) {
    const { name, ext, hash, size, mtime, height, width, duration } =
      files[type];
    newFiles[type] = {
      name,
      ext,
...
async function getRemoteFile(props) {
  const { id, toc, type, ext, file } = props;
