...
fn.checkNumberOfSendMessage = async (nationCode, mobile, type) => {
  ...
  let smsCodes = await db.SmsCodeModel.find({
    nationCode,
    mobile,
    type,
    toc: { $gt: time },
...
fn.checkNumberOfSendEmail = async (email) => {
  let time = Date.now() - 24 * 60 * 60 * 1000;
  let emailOfDB = await db.EmailRegisterModel.find({
    email: email,
    toc: { $gt: time },
  });
...
fn.checkNumberOfSendEmailReset = async (email) => {
  let time = Date.now() - 24 * 60 * 60 * 1000;
  let emailOfDB = await db.EmailCodeModel.find({
    email: email,
    toc: { $gt: time },
  });
  return emailOfDB.length;
...
fn.checkUsername = async (username) => {
  let usernameOfDB = await db.UserModel.find({
...
fn.checkMobileCode = async (nationCode, mobile, code) => {
  ...
  return await db.SmsCodeModel.findOne({
...
fn.checkEmailCode = async (email, code) => {
  ...
  return await db.EmailRegisterModel.findOne({
    email: email,
    ecode: code,
    toc: { $gt: time },
    used: false,
...
fn.useRegCode = async (regCode, uid) => {
  return await db.AnswerSheetModel.replaceOne(
    { key: regCode },
    { $set: { uid: uid } },
  );
};
...
fn.checkRegisterCode = async (regCode) => {
  ...
  if (Date.now() - regCodeOfDB.tsm > settings.exam.timeBeforeRegister) {
    throw '答卷的注册码过期，可能要重新参加考试';
  }
  return regCodeOfDB;
};
...
fn.forumsListSort = (forums, types) => {
  ...
  for (let forum of forums) {
    ...
    for (const fid of forum.parentsId) {
      ...
      if (parentForum) {
        parentForum.childrenForums = parentForum.childrenForums
          ? parentForum.childrenForums
          : [];
