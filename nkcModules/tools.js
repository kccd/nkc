...
var Tools = function () {
  ...
  self.briefTime = function (toc) {
    ...
    if (timeNumber >= t - oneDayNumber) {
      return '昨天';
    }
    if (timeNumber >= t - 2 * oneDayNumber) {
      return '前天';
    }
    if (timeNumber >= t - 30 * oneDayNumber) {
      return '近期';
    }
    return '较早';
  };
  ...
  self.createVueAppSelector = function () {
    ...
    for (var i = 0; i < 8; i++) {
      str[i] =
        elementIdChars[Math.floor(Math.random() * elementIdChars.length - 1)];
    }
    return str.join('');
  };
  ...
  self.removeLink = function (content) {
    content = content || '';
    var reg = /(https?:\/\/)?([-0-9a-zA-Z]{1,256}\.)+[a-zA-Z]{2,6}/gi;
