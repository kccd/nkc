...
function APP_nkc_render(options){
  var render = {};
  var commonmark;
  var plain_escape;
  var XBBCODE;
  ...
  default_whitelist.embed = ['width','height','src','frameborder','allowfullscreen'];
	default_whitelist.img = ['src','style','class'];
	for(var i = 1; i <= 6; i++) {
  ...
  var pwbb_experimental = function(post,isHTML){
    ...
    //     if(count>=50)throw '[APP_nkc_render]too much attachment included! refuse to process.'
  ...
  render.experimental_render = function(post){
    ...
    renderedHTML = renderedHTML.replace(/<img(.*?)src="(.*?)"(.*?)>/igm,"<img$1src='$2'$3 ontouchend='touchend()' ontouchmove='touchend()' ontouchstart='touchstart(\"$2\")' onclick='openImage(\"$2\")'>");    
    // 将视频替换成图片
    renderedHTML = renderedHTML.replace(/<video(.*?)src=".*?\/r(.*?)".*?>(.*?)<\/video>/igm,'<div style="position:relative"><img src="http://192.168.11.114:9000/frameImg$2" style="width:100%;height:10rem"><span class="play-btn" onclick="openVideo(this,\'/r$2\')"></span></div>');
