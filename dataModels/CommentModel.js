...
schema.statics.checkCommentSource = async (source) => {
  ...
  if (!sourceNames.includes(source)) {
    throwErr(500, `comment source error. source=${source}`);
  }
};
...
schema.statics.createComment = async (options) => {
  ...
  const { comment: commentDocumentSource } =
...
schema.methods.publishComment = async function (
  ...
  if (newComment.status === normalStatus) {
    ...
    // 如果需要审核就将渲染好的内容返回
    const singleCommentData = await CommentModel.getSocketSingleCommentData(
      this._id,
    );
    renderedComment = {
...
schema.statics.extendPostComments = async (props) => {
  ...
  for (const r of kcbsRecords) {
    uidArr.push(r.from);
    r.to = '';
    if (!kcbsRecordsObj[r.commentId]) {
      kcbsRecordsObj[r.commentId] = [];
    }
  ...
  for (const d of documents) {
    ...
    if (!permissions.reviewed) {
      if (d.uid !== uid) {
        if (
          (d.status !== normalStatus || d.type !== stableType) &&
          !isModerator
        ) {
          continue;
        }
  ...
  for (const document of documents) {
    if (!document.quoteDid || !documentObj[document.did]) {
      continue;
    }
  ...
  for (const c of comments) {
    ...
    if (uid) {
      result.vote = await PostsVoteModel.getVoteByUid({
        uid,
        type: 'comment',
        id: c._id,
      });
...
schema.statics.extendReviewComments = async function (comments) {
  ...
  //查找评论的引用
  const articlePosts = await ArticlePostModel.find({
    _id: { $in: articlePostSid },
  });
  ...
  for (const d of commentDocuments) {
    const { type, sid } = d;
    if (!commentsDocumentObj[sid]) {
      commentsDocumentObj[sid] = {};
    }
  ...
  for (const comment of comments) {
    ...
    if (!commentObj || !articleDocumentObj) {
      continue;
    }
...
schema.methods.extendEditorComment = async function () {
  ...
  const { htmlToPlain } = require('../nkcModules/nkcRender');
  const { comment: commentSource } = await DocumentModel.getDocumentSources();
  const { beta: betaType, stable: stableType } =
    await DocumentModel.getDocumentTypes();
  const { did, _id, toc, uid, sid } = this;
  const documents = await DocumentModel.find({
    did,
    type: { $in: [betaType, stableType] },
    source: commentSource,
    sid: _id,
  });
  const commentsObj = {};
 ...
 * */
schema.statics.extendDocumentOfComment = async function (
  comments,
  type = 'beta',
  options,
) {
...
schema.statics.getCommentsByCommentsId = async function (commentsId) {
  ...
  const results = {};
  for (const comment of comments) {
    const { status, did, _id, commentDocument, articleDocument, url } = comment;
...
schema.methods.noticeAuthorComment = async function () {
  ...
  Promise.resolve()
    .then(async () => {
      ...
      if (commentDocument.uid === articleDocument.uid) {
        return;
      }
      //去通知文章作者文章被回复
    ...
    .then(async () => {
      ...
      if (commentDocument.quoteDid) {
        //获取被引用回复的信息
        const quoteDocument = await DocumentModel.findOnly({
          _id: commentDocument.quoteDid,
        });
        if (!quoteDocument) {
          return console.log('未找到评论引用信息document');
        }
        const quoteComment = await CommentModel.findOnly({
          _id: quoteDocument.sid,
        });
        if (!quoteComment) {
          return console.log('未找到评论引用信息comment');
        }
        const quoteInfo = (
          await CommentModel.getCommentsInfo([quoteComment])
        )[0];
        if (!quoteInfo) {
          return console.log('未找到评论引用信息');
        }
