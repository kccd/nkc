...
usersPersonalSchema.methods.extendIdPhotos = async function () {
  ...
  let idCardA = await PhotoModel.findOne({
    uid: this.uid,
    type: 'idCardA',
  }).sort({ toc: -1 });
  let idCardB = await PhotoModel.findOne({
    uid: this.uid,
    type: 'idCardB',
  }).sort({ toc: -1 });
  let handheldIdCard = await PhotoModel.findOne({
    uid: this.uid,
    type: 'handheldIdCard',
  }).sort({ toc: -1 });
...
usersPersonalSchema.methods.getAuthLevelFromPhoto = async function () {
  ...
  if (
    !(
      idCardA &&
      idCardA.status === 'passed' &&
      idCardB &&
      idCardB.status === 'passed'
    )
  ) {
    return 1;
  }
...
usersPersonalSchema.methods.extendLifePhotos = async function () {
  const PhotoModel = require('./PhotoModel');
  const lifePhotos = await PhotoModel.find({
    uid: this.uid,
    type: 'life',
    status: { $ne: 'deleted' },
  }).sort({ toc: -1 });
...
usersPersonalSchema.methods.extendCertsPhotos = async function () {
  const PhotoModel = require('./PhotoModel');
  const certsPhotos = await PhotoModel.find({
    uid: this.uid,
    type: 'cert',
    status: { $ne: 'deleted' },
  }).sort({ toc: -1 });
...
usersPersonalSchema.statics.findUsersPersonalById = async (uid) => {
  ...
  if (!personal) {
    throwErr(404, `未找到ID为【${uid}】的用户隐私信息`);
  }
  return personal;
};
...
usersPersonalSchema.statics.shouldVerifyPhoneNumber = async function (uid) {
  ...
  const UsersPersonalModel = mongoose.model('usersPersonal');
  const userPersonal = await UsersPersonalModel.findOne(
    { uid },
    { lastVerifyPhoneNumberTime: 1, numberOfVerifications: 1 },
  );
  if (!userPersonal) {
    return false;
  }
  const authSettings = await SettingModel.getSettings('auth');
  if (!authSettings.verifyPhoneNumber.enabled) {
    return false;
  }
  if (!userPersonal.lastVerifyPhoneNumberTime) {
    return true;
  }
  ...
  if (Date.now() - lastVerifyPhoneNumberTime.getTime() > maxInterval) {
    ...
    return (
      Date.now() - lastVerifyPhoneNumberTime.getTime() > interval + extendedTime
    );
  }
};
...
usersPersonalSchema.statics.modifyUserPhoneNumber = async (props) => {
  const { uid, phoneNumber, ip, port } = props;
  const UsersPersonalModel = mongoose.model('usersPersonal');
 ...
 * 生成身份认证2记录
 */
usersPersonalSchema.methods.generateAuthenticateVerify2 = async function (
  files,
) {
  ...
  const pictureAId = await VerifiedUploadModel.saveIdentityInfo(
    this.uid,
    imageA,
    'identityPictureA',
  );
  const pictureBId = await VerifiedUploadModel.saveIdentityInfo(
    this.uid,
    imageB,
    'identityPictureB',
  );
 ...
 * 生成身份认证3记录
 */
usersPersonalSchema.methods.generateAuthenticateVerify3 = async function (
  file,
  code,
) {
  if (!file) {
  ...
  const VerifiedUploadModel = mongoose.model('verifiedUpload');
  const aid = await VerifiedUploadModel.saveIdentityInfo(
    this.uid,
    file,
    'identityVideo',
  );
...
usersPersonalSchema.methods.changeVerify2Status = async function (status) {
  ...
  if (!valueList.includes(status)) {
    throw new Error(
      `must incoming value of status is one of [${valueList.join(', ')}]`,
    );
  }
  await this.updateOne({
    $set: {
...
usersPersonalSchema.statics.checkExpiryAuthenticate = async function (uid) {
  ...
  if (
    ...
    authenticate.video.expiryDate.getTime() < Date.now()
  ) {
    await userPersonal.updateOne({
  ...
  if (
    ...
    authenticate.card.expiryDate.getTime() < Date.now()
  ) {
    await userPersonal.updateOne({
...
usersPersonalSchema.statics.getUserSecuritySettings = async (uid) => {
  const UsersPersonalModel = mongoose.model('usersPersonal');
  const usersPersonal = await UsersPersonalModel.findOnly(
    { uid },
    {
      password: 1,
      email: 1,
...
usersPersonalSchema.statics.getUserAuth3Code = async function (uid) {
  const UsersPersonalModel = mongoose.model('usersPersonal');
  const { auth3Code } = await UsersPersonalModel.findOnly(
    { uid },
    {
      auth3Code: 1,
    },
  ...
  if (
    auth3Code.t === null ||
    now.getTime() > auth3Code.t.getTime() + 6 * 60 * 60 * 1000
  ) {
    auth3Code.t = now;
    // 如果没有获取过或距离上次获取身份认证动态码已超过6小时，则需要更新动态码
    // 由于最多仅允许保留最新的5个动态码，所以需要保留后4个然后再生成一个
    auth3Code.c = auth3Code.c.slice(-4);
    auth3Code.c.push(apiFunction.getRandomString('0', 4));
    await UsersPersonalModel.updateOne(
      { uid },
      {
        $set: {
          'auth3Code.t': auth3Code.t,
          'auth3Code.c': auth3Code.c,
        },
      },
    );
  }
 ...
 * 用户获取自己的身份认证动态码
 * */
usersPersonalSchema.statics.getUserOwnAuth3Code = async (uid) => {
  const UsersPersonalModel = mongoose.model('usersPersonal');
  const { t, c } = await UsersPersonalModel.getUserAuth3Code(uid);
  return {
    time: t,
    code: c[c.length - 1],
  };
 ...
 * 管理员获取用户的身份认证动态码
 * */
usersPersonalSchema.statics.adminGetUserAuth3Codes = async (uid) => {
  const UsersPersonalModel = mongoose.model('usersPersonal');
  const { c: codes } = await UsersPersonalModel.getUserAuth3Code(uid);
  return codes;
};
