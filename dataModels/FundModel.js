...
fundSchema.methods.getUserFundRoles = async function (uid) {
  ...
  for (const roleId in defaultFundRoles) {
    if (!defaultFundRoles.hasOwnProperty(roleId)) {
      continue;
    }
...
fundSchema.methods.ensureOperatorPermission = async function (type, user) {
  if (!user) {
    return false;
  }
  const { expert, censor, financialStaff, admin, commentator, voter } = this;
  ...
  switch (type) {
    case 'expert':
      targetOptions = expert;
      break;
    case 'censor':
      targetOptions = censor;
      break;
    case 'financialStaff':
      targetOptions = financialStaff;
      break;
    case 'admin':
      targetOptions = admin;
      break;
    case 'commentator':
      targetOptions = commentator;
      break;
    case 'voter':
      targetOptions = voter;
      break;
  ...
  const userRolesId = await user.getRolesId();

  for (let cert of targetOptions.certs) {
    if (userRolesId.includes(cert)) {
      return true;
    }
  }
  return targetOptions.appointed.includes(user.uid);
};
...
fundSchema.methods.getConflictingByUser = async function (userId) {
  ...
  if (count >= this.applicationCountLimit) {
    return { info: '你今年申报当前基金的次数已用尽' };
  }
  ...
  const applicationForms = await FundApplicationFormModel.find({
    uid: { $ne: userId },
    _id: { $in: applicationFormUsers.map((a) => a.applicationFormId) },
  });
  ...
  if (!this.canApply) {
    return { info: '基金暂不接受新的申请' };
  }
  if (this.history) {
    return { info: '当前基金已被设为历史基金，不再接受新的申请' };
  }
...
fundSchema.statics.getConditionsOfApplication = async (userId, fundId) => {
  ...
  const { userLevel, threadCount, postCount, timeToRegister, authLevel } =
    fund.applicant;
  ...
  const userTimeToRegister = Math.round(
    (Date.now() - new Date(user.toc).getTime()) / (1000 * 60 * 60 * 24),
  );
  ...
  const table = [
    ...
    [
      ...
      timeToRegister,
      userTimeToRegister,
      userTimeToRegister >= timeToRegister,
    ],
...
fundSchema.statics.checkAccessControlPermissionWithThrowError = async (
  props,
) => {
