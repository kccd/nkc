...
schema.statics.createAccessControl = async (props) => {
  const { source, sid = '', app, web } = props;
...
schema.statics.checkAccessControlPermission = async (props) => {
  const { uid, rolesId, gradeId, isApp, source, sid = '' } = props;
  ...
  if (!['app', 'web'].includes(platform)) {
    throwErr(500, `访问控制未知平台 platform=${platform}`);
  }
  ...
  // 当前用户为指定的特殊用户
  if (!!uid && targetAC.whitelist.usersId.includes(uid)) {
    return;
  }

  let hasRole = false;
  for (const roleId of rolesId) {
    if (!targetAC.whitelist.rolesId.includes(roleId)) {
      continue;
    }
    hasRole = true;
    break;
  ...
  // 当关系为 or 时，拥有证书或等级
  if (
    (targetAC.whitelist.relation === 'and' && hasRole && hasGrade) ||
    (targetAC.whitelist.relation === 'or' && (hasRole || hasGrade))
  ) {
    return;
  }
