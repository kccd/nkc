...
schema.statics.getDataRequiredForArticle = async (columnId, _id, xsf) => {
  ...
  const { article: articleType, thread: threadType } =
    await ColumnPostModel.getColumnPostTypes();
  let res = {};
  if (articleData.type === articleType) {
    ...
    res = {
      ...
      post: {
        ...
        c: await ColumnPostModel.getRenderHTML(
          content,
 ...
 * 获取渲染的富文本
 * */
schema.statics.getRenderHTML = async function (
  content,
  documentResourceId,
  xsf,
  atUsers = [],
...
schema.statics.extendColumnPosts = async (options) => {
  ...
  for (let p of columnPosts) {
    ...
    if (p.type === 'thread') {
      ...
      p.thread.firstPost.coverUrl = getUrl(
        'postCover',
        p.thread.firstPost.cover,
      );
...
schema.statics.getCategoriesOrder = async (categoriesId) => {
  ...
  const order = {};
  order[`cid_default`] = await SettingModel.operateSystemID(
    'columnPostOrders',
    1,
  );
  for (const _id of categoriesId) {
    order[`cid_${_id}`] = await SettingModel.operateSystemID(
      'columnPostOrders',
      1,
    );
  }
...
schema.statics.getToppedColumnPosts = async (options) => {
  ...
  for (const _id of columnPostsId) {
    ...
    if (columnPost) {
      columnPosts.push(columnPost);
    }
  }
  return await ColumnPostModel.extendColumnPosts({
    columnPosts,
    fidOfCanGetThread,
    user,
    isModerator,
 ...
 * */
schema.statics.addColumnPosts = async (
  columnId,
  categoriesId,
  minorCategoriesId,
  postsId,
  ...
  for (const _id of categoriesId) {
    ...
    if (c) {
      categoriesId_.push(_id);
    }
  }
  for (const _id of minorCategoriesId) {
    const c = await ColumnPostCategoryModel.findOne({ _id, columnId });
    if (c) {
      minorCategoriesId_.push(_id);
    }
  }
...
schema.post('save', async function (columnPost) {
  if (columnPost.type === 'thread') {
    await mongoose.model('threads').updateOne(
      { oc: columnPost.pid },
      {
        $set: {
          inColumn: true,
...
schema.post('remove', async function (columnPost) {
  ...
  const column = await mongoose
    .model('columns')
    .findOne({ _id: columnPost.columnId });
  if (column) {
    await column.updateBasicInfo();
  }
});
