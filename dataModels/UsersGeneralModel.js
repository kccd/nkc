...
usersGeneralSchema.statics.updateThreadAccessLogs = async (uid, tid) => {
  ...
  const visitedThreads = await UsersGeneralModel.replaceItem(
    userGeneral.visitedThreads,
    tid,
  );
...
usersGeneralSchema.statics.updateUserAccessLogs = async (uid, tUid) => {
  ...
  const visitorVisitedUsers = await UsersGeneralModel.replaceItem(
    visitor.visitedUsers,
    tUid,
  );
  const targetAccessedUsers = await UsersGeneralModel.replaceItem(
    target.accessedUsers,
    uid,
  );
...
usersGeneralSchema.statics.resetReviewedCount = async function (uid, type) {
  ...
  const review = await SettingModel.findOne(
    { _id: 'review' },
    {
      article: 1,
      document: 1,
  ...
  if (type.includes('article')) {
    await UsersGeneralSchema.updateOne(
      { uid },
      {
        $set: {
...
usersGeneralSchema.statics.getUserVisitedForumsId = async (uid) => {
  const UsersGeneralModel = mongoose.model('usersGeneral');
  const generalSettings = await UsersGeneralModel.findOne(
    { uid },
    {
      visitedForumsId: 1,
    },
