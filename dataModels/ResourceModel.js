...
resourceSchema.methods.setFileExist = async function (excludedMediaTypes = []) {
  ...
  if (excludedMediaTypes.includes(mediaType)) {
    return;
  }
  ...
  // 判断游客是否允许访问指定类型的资源
  if (
    ['mediaVideo', 'mediaAudio', 'mediaAttachment'].includes(this.mediaType)
  ) {
...
resourceSchema.statics.getMetadata = async function (resources) {
  ...
  for (const resource of resources) {
    ...
    if (mediaType === 'mediaAudio' || mediaType === 'mediaVideo') {
      ...
      if (!requestFiles[storeUrl]) {
        requestFiles[storeUrl] = [];
      }
  ...
  for (const storeUrl in requestFiles) {
    ...
    const { files: storeFiles } = await FILE.getMetaInformation(
      storeUrl,
      files,
    );
...
resourceSchema.statics.toReferenceSource = async function (id, declare = '') {
  ...
  $(`[data-tag="nkcsource"]`).each((index, el) => {
    ...
    if (!['picture', 'video', 'audio', 'attachment'].includes(type)) {
      return;
    }
    resourcesId.push(id);
  });
 ...
 * */
resourceSchema.statics.replaceReferencesById = async function (
  resourcesId,
  referenceId,
) {
  const ResourceModel = mongoose.model('resources');
...
resourceSchema.statics.checkUploadPermission = async (user, file) => {
  ...
  let { size, ext } = file;
  if (!ext) {
    ext = await getFileExtension(file);
  }
  // 检查文件大小是否符合要求
  let settingSize;
  ...
  const uploadedCount = await ResourceModel.countDocuments({
    uid: user.uid,
    toc: { $gte: today },
  });
  const certList = [];
  ...
  if (uploadedCount >= settingCount) {
    throwErr(400, '今日上传文件数量已达上限');
  }

  // 检查文件格式黑白名单
  let blacklistArr = undefined,
    whitelistArr = undefined;
  ...
  if (blacklistArr === undefined) {
    // 仅存白名单
    if (!whitelistArr.includes(ext)) {
      throwErr(400, '文件格式不被允许');
    }
  } else if (whitelistArr === undefined) {
    // 仅存黑名单
    if (blacklistArr.includes(ext)) {
      throwErr(400, '文件格式不被允许');
    }
  } else {
    // 黑白名单均有
...
resourceSchema.methods.checkDownloadPermission = async function (user, ip) {
  ...
  if (fileCount === 0) {
    if (!user) {
      ...
      throwErr(
        403,
        `当前时段（${startingTime}点 - ${endTime}点）暂不允许下载附件，请下一个时段再试。`,
      );
    }
  }
  let downloadLogs;
  ...
  if (!downloadToday) {
    ...
    if (downloadLogs.length >= fileCount) {
      if (user) {
        throwErr(
          403,
          `当前时段（${startingTime}点 - ${endTime}点）下载的附件数量已达上限，请在下一个时段再试。`,
        );
      } else {
        throwErr(
          403,
          `未登录用户当前时段（${startingTime}点 - ${endTime}点）只能下载${fileCount}个附件，请登录或注册后重试。`,
        );
      }
    }
  }
...
resourceSchema.methods.checkDownloadCost = async function (user) {
  ...
  const todayOperationCount = await ScoreOperationLogModel.getOperationLogCount(
    user,
    'attachmentDownload',
  );
  const lastAttachmentDownloadLog =
    await ScoreOperationLogModel.getLastAttachmentDownloadLog(user, this.rid);
  const nowTime = new Date();
  let description = '';
  if (lastAttachmentDownloadLog) {
    const lastAttachmentDownloadTime = lastAttachmentDownloadLog.toc;
    const timeout =
      nowTime - lastAttachmentDownloadTime - freeTime * 60 * 60 * 1000;
    if (timeout <= 0) {
      // 下载时间未超过24小时 不收费
      return {
        needScore: false,
      ...
      description = `您曾于${moment(lastAttachmentDownloadTime).format(
        'YYYY年MM月DD日',
      )}下载该附件，现已超过${Math.ceil(
        timeout / (60 * 60 * 1000),
      )}小时，再次下载将花费积分。`;
    }
  }
 ...
 * */
resourceSchema.methods.checkAccessPermission = async function (
  accessibleForumsId,
) {
...
resourceSchema.methods.pushToMediaService = async function (filePath) {
  ...
  //获取水印信息
  if (data.waterAdd) {
    coverPath = await SettingModel.getWatermarkCoverPathByUid(
      uid,
      data.waterType,
    );
  }
...
resourceSchema.methods.getMediaServiceDataPicture = async function () {
  ...
  return {
    ...
    transparency: watermarkSettings.transparency / 100,
    waterType: 'picture',
    waterAdd,
...
resourceSchema.methods.getRemoteFile = async function (size) {
  ...
  if (prid) {
    const parentResource = await ResourceModel.findOne({ rid: prid });
    if (parentResource) {
      return await parentResource.getRemoteFile(size);
    }
  }
 ...
 * */
resourceSchema.statics.getResourcesObjectByResourcesId = async (
  resourcesId,
) => {
  const ResourceModel = mongoose.model('resources');
