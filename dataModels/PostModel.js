...
postSchema.methods.ensurePermissionNew = async function (options) {
  ...
  if (this.disabled) {
    if (!isModerator) {
      if (
        this.toDraft &&
        !userOperationsId.includes('displayRecycleMarkThreads')
      ) {
...
postSchema.post('save', async function (doc, next) {
  ...
  try {
    ...
    const notInformedUsers = atUsers.filter(
      (at) =>
        !oldAtUsers // map the user not in oldAtUsers
          .find((oldAt) => oldAt.uid === at.uid),
    );
    await Promise.all(
      notInformedUsers.map(async (at) => {
...
postSchema.statics.extendPosts = async (posts, options) => {
  ...
  if (o.credit) {
    ...
    for (const r of kcbsRecords) {
      uid.add(r.from);
      r.to = '';
      if (!kcbsRecordsObj[r.pid]) {
        kcbsRecordsObj[r.pid] = [];
      }
  ...
  for (let post of posts) {
    ...
    // 是否为文章作者
    post.isAuthor = !post.anonymous
      ? post.uid === threadsAuthor[post.tid]
      : false;
    results.push(post);
  }
  return results;
...
postSchema.statics.newPost = async (options) => {
  ...
  if (contentLength(title) > 200) {
    throwErr(400, '标题不能超过200字节');
  }
  if (!content) {
    throwErr(400, '内容不能为空');
  }
  if (contentLength(content) < 6) {
    throwErr(400, '内容太短了，至少6个字节');
  }
...
postSchema.statics.ensureAttachmentPermission = async function (uid) {
  ...
  if (!uid) {
    return rolesId.includes('visitor');
  }
  const user = await mongoose.model('users').findOne({ uid });
  if (!user) {
    return rolesId.includes('visitor');
...
postSchema.statics.ensureHidePostPermission = async (thread, user) => {
  ...
  if (allowedAuthor && thread.uid === user.uid) {
    return true;
  }
  if (!user.roles) {
    await user.extendRoles();
  ...
  for (const role of user.roles) {
    if (allowedRolesId.includes(role._id)) {
      return true;
    }
  }
  return false;
  ...
  for (const post of posts) {
    ...
    if (post.quotePost) {
      quote = {
        ...
        username: post.quotePost.uid
          ? post.quotePost.username
          : anonymousUser.username,
        userHome: post.quotePost.uid
          ? tools.getUrl('userHome', post.quotePost.uid)
          : '',
    ...
    const result = {
      ...
      toc: post.toc,
      tlm:
        new Date(post.toc).getTime() === new Date(post.tlm).getTime()
          ? null
          : post.tlm,
...
postSchema.statics.extendActivityPosts = async (posts) => {
  ...
  const postOfThreads = await ThreadModel.find({
    tid: { $in: [...postOfThreadsId] },
  });
  ...
  const firstPosts = await PostModel.find(
    { pid: { $in: [...firstPostsId] } },
    {
      tid: 1,
      uid: 1,
...
postSchema.statics.checkPostCommentPermission = async (pid, type) => {
  const PostModel = mongoose.model('posts');
  let post = await PostModel.findOnly(
    { pid },
    { parentPostId: 1, parentPostsId: 1, pid, comment: 1 },
  );
  if (post.type === 'thread') {
    return false;
  }
  if (post.parentPostsId.length > 0) {
    post = await PostModel.findOnly(
      { pid: post.parentPostsId[0] },
      { comment: 1 },
    );
  }
  if (!['read', 'write'].includes(type)) {
    throwErr(500, `评论控制类型错误 type: ${type}`);
  }
  if (type === 'read') {
...
postSchema.statics.getPostsDataByPostsId = async (postsId, uid) => {
  ...
  let threadFirstPosts = await PostModel.find(
    { pid: { $in: threadsOC } },
    {
      pid: 1,
      tid: 1,
  ...
  const results = {};
  for (const post of posts) {
    const { tid, pid, type, mainForumsId = [], disabled, toDraft } = post;
...
postSchema.methods.noticeAuthorReply = async function () {
  ...
  let { type, pid, tid, reviewed, disabled, toDraft, quote, parentPostId } =
    newPost;
  if (type === 'thread') {
    return;
  }
  ...
  if (reviewed) {
    Promise.resolve()
      .then(async () => {
        ...
        if (quote) {
          let quotePost = await PostModel.findOne(
            { tid, pid: quote.split(':').shift(), type: 'post' },
            { cv: 1, pid: 1, uid: 1 },
          );
 ...
 * */
postSchema.statics.insertSystemRecord = async (
  type,
  user,
  ctx,
  additionalReward,
  ...
  for (const enabledScore of enabledScores) {
    ...
    if (type === 'digestPost' || type === 'digestThread') {
      ...
      const digestType =
        type === 'unDigestPost' ? 'digestPost' : 'digestThread';
      const match = {
        type: digestType,
        to: user.uid,
        pid: data.post.pid,
