...
const fundApplicationFormSchema = new Schema(
  {
    ...
    */
    ...
    status: {
      submitted: {
        // 判断是否是提交过的申请表，下边的lock.submitted作用是判断申请表当前是否提交。
        type: Boolean,
        default: null,
      },
      usersSupport: {
...
fundApplicationFormSchema.pre('save', async function (next) {
  ...
  if (submitted && !code) {
    ...
    const a = await FundApplicationFormModel.findOne({
      fundId: fund._id,
      year,
    }).sort({ order: -1 });
    let code, order;
...
fundApplicationFormSchema.methods.extendApplicant = async function (
  ...
  const applicant = await FundApplicationUserModel.findOne({
  ...
  if (applicant) {
    ...
    applicant.inFundBlacklist = await FundBlacklistModel.inBlacklist(
      applicant.uid,
    );
  }
...
fundApplicationFormSchema.methods.extendMembers = async function () {
  ...
  return (this.members = await Promise.all(
    applicationUsers.map(async (aUser) => {
...
fundApplicationFormSchema.methods.extendThreads = async function () {
  ...
  const threads = {};
  threads.applying = await Promise.all(
    threadsId.applying.map(async (tid) => {
  ...
  threads.completed = await Promise.all(
    threadsId.completed.map(async (tid) => {
      const thread = await ThreadModel.findOnly({ tid });
      const firstPost = await thread.extendFirstPost();
      if (firstPost.anonymous) {
        thread.uid = '';
...
fundApplicationFormSchema.methods.extendReportThreads = async function () {
  ...
  const reportThreads = await Promise.all(
    threadsId.map(async (tid) => {
      const thread = await ThreadModel.findOnly({ tid });
      const firstPost = await thread.extendFirstPost();
      if (firstPost.anonymous) {
        thread.uid = '';
...
fundApplicationFormSchema.statics.publishByApplicationFormId = async (
  ...
  oldForm.comments = await FundDocumentModel.find({
    applicationFormId: form._id,
    type: 'comment',
  });
  oldForm.adminAudit = await FundDocumentModel.findOne({
    applicationFormId: form._id,
    type: 'adminAudit',
  }).sort({ toc: -1 });
  oldForm.userInfoAudit = await FundDocumentModel.findOne({
    applicationFormId: form._id,
    type: 'userInfo',
  }).sort({ toc: -1 });
  oldForm.projectAudit = await FundDocumentModel.findOne({
    applicationFormId: form._id,
    type: 'projectAudit',
  }).sort({ toc: -1 });
  oldForm.moneyAudit = await FundDocumentModel.findOne({
    applicationFormId: form._id,
    type: 'moneyAudit',
  }).sort({ toc: -1 });
  ...
  let fundForumsId = fundForums.map((f) => f.fid);
  if (!fundForumsId.includes(form.category)) {
    fundForumsId.unshift(form.category);
  }
  // 生成文章
  let formThread;
  if (form.tid) {
...
fundApplicationFormSchema.statics.extendAsApplicationFormList = async (
  ...
  const projects = await FundDocumentModel.find(
    { _id: { $in: projectsId } },
    {
      t: 1,
      _id: 1,
  ...
  members.map((m) => {
    ...
    if (!applicationFormToUser[applicationFormId]) {
      applicationFormToUser[applicationFormId] = [];
    }
...
fundApplicationFormSchema.methods.updateMoney = async function () {
  ...
  if (fixedMoney) {
    ...
    const { general } = await this.getStatus();
    let result = 0;
...
fundApplicationFormSchema.statics.updateMoneyByApplicationFormId = async (
  applicationFormId,
) => {
  const FundApplicationFormModel = mongoose.model('fundApplicationForms');
  const applicationForm = await FundApplicationFormModel.findOnly({
    _id: applicationFormId,
  });
...
fundApplicationFormSchema.methods.updateProject = async function (props) {
  const { t, c, abstractEn, abstractCn, keyWordsCn, keyWordsEn } = props;
  const FundDocumentModel = mongoose.model('fundDocuments');
  if (!this.projectId) {
    // 初始化项目信息
 ...
 * */
fundApplicationFormSchema.methods.createReport = async function (
  type,
  content,
  operatorId,
  support = null,
 ...
 * */
fundApplicationFormSchema.methods.extendReports = async function (
  isAdmin = false,
) {
  /*const FundDocumentModel = mongoose.model('fundDocuments');
  const match = {
  ...
  return (this.reports =
    await fundOperationService.getTimelineOperationsDetailInfoByFormId(
      this._id,
      isAdmin,
    ));
};
...
fundApplicationFormSchema.methods.extendProjectPost = async function () {
  ...
  if (this.tid) {
    const firstPost = await PostModel.findOnly({
      tid: this.tid,
      type: 'thread',
    });
 ...
 * */
fundApplicationFormSchema.methods.checkRemittanceNumber = async function (
  number,
) {
  const FundApplicationFormModel = mongoose.model('fundApplicationForms');
  const form = await FundApplicationFormModel.findOnly(
  ...
  for (let i = 0; i < remittance.length; i++) {
    ...
    if (i < number) {
      if (!status) {
        throwErr(403, `第 ${i + 1} 期拨款尚未完成`);
      }
      if (!verify) {
        throwErr(403, `第 ${i + 1} 期拨款尚未确认`);
      }
    } else if (i === number) {
      if (status) {
        throwErr(403, `拨款已完成`);
 ...
 * 拓展回复
 * */
fundApplicationFormSchema.methods.extendPosts = async function (
  accessForumsId,
) {
  const PostModel = mongoose.model('posts');
  let posts = [];
 ...
 * 拓展申请表信息
 * */
fundApplicationFormSchema.methods.extendApplicationFormBaseInfo =
  async function () {
    await this.extendFund();
    await this.extendMembers();
 ...
 * 拓展基金的回复等
 * */
fundApplicationFormSchema.methods.extendApplicationFormInfo = async function (
  uid,
  accessForumsId = [],
) {
  const { fund } = this;
  if (this.disabled && !(await fund.isFundRole(uid, 'admin'))) {
    throwErr(403, `申请表已被屏蔽`);
  }
  const { applicant, members } = this;
  const membersId = members.map((m) => m.uid);
  // 未提交时仅自己和全部组员可见
  const isAdmin = await fund.isFundRole(uid, 'admin');
  if (
    !isAdmin &&
    this.status.submitted !== true &&
    uid !== applicant.uid &&
  ...
  await this.extendReports(isAdmin);
 ...
 * */
fundApplicationFormSchema.methods.hideApplicationFormInfoByUserId =
  async function (uid, hasPermission) {
    const { fund } = this;
    const isAdmin =
      hasPermission ||
 ...
 * */
fundApplicationFormSchema.methods.withdraw = async function (
  uid,
  reason,
  applicant,
) {
  ...
  if (this.remittance[0].status === true) {
    throwErr(400, '无法撤回已拨款的基金申请');
  }
 ...
 * 人工设置基金申请退款状态
 * */
fundApplicationFormSchema.methods.adminUpdateRefundStatus = async function (
  operatorId,
) {
  if (this.status.refund !== false) {
    return;
  }
  const FundBillModel = mongoose.model('fundBills');
...
const FundApplicationFormModel = mongoose.model(
