...
schema.statics.checkDocumentSource = async (source) => {
  ...
  if (!sourceNames.includes(source)) {
    ThrowServerInternalError(`document source error. source=${source}`);
  }
};
...
schema.statics.checkDocumentStatus = async (status) => {
  ...
  if (!typeNames.includes(status)) {
    ThrowServerInternalError(`document status error. status=${status}`);
  }
};
...
schema.statics.checkDocumentType = async (type) => {
  ...
  if (!typeNames.includes(type)) {
    ThrowServerInternalError(`document type error. type=${type}`);
  }
};
...
schema.methods.copyToHistoryDocument = async function (status) {
  ...
  await document.save();
  await NoteModel.copyDocumentNoteAndUpdateNewNoteTargetId(
    this._id,
    document._id,
  );
  return document;
 ...
 *
 */
schema.statics.copyToHistoryToEditDocument = async function (
  uid,
  sid,
  source,
  _id,
 ...
 * */
schema.statics.checkContentAndCopyBetaToHistoryBySource = async (
  source,
  sid,
) => {
  const DocumentModel = mongoose.model('documents');
  ...
  const time = Date.now();
  const latestHistoryDocument =
    await DocumentModel.getLatestHistoryDocumentBySource(source, sid);
...
schema.statics.createBetaDocumentByStableDocument = async function (did) {
  ...
  if (!stableDocument) {
    ThrowServerInternalError(`文档 ${did} 不存在正式版，无法生成编辑版`);
  }
  ...
  await NoteModel.copyDocumentNoteAndUpdateOriginNoteTargetId(
    stableDocument._id,
    betaDocument._id,
  );
  return betaDocument;
...
schema.statics.publishDocumentByDid = async (did, options = {}) => {
  ...
  if (!documentsObj.beta) {
    ThrowCommonError(400, `不存在编辑版，无需发表`);
  }
  //判断专栏文章编辑版是否填写标题
  ...
  //是否需要审核
  const needReview = jumpReview
    ? false
    : await documentsObj.beta.getReviewStatusAndCreateReviewLog();
  if (needReview) {
...
schema.statics.updateDocumentByDid = async (did, props) => {
  ...
  if (resourcesId) {
    updateObject.files = resourcesId;
  }
...
schema.statics.getStableDocumentContentBySource = async (source, sid) => {
  const DocumentModel = mongoose.model('documents');
  let stableDocument = await DocumentModel.getStableDocumentBySource(
    source,
    sid,
  );
  if (!stableDocument) {
...
schema.statics.getEditorBetaDocumentContentBySource = async (source, sid) => {
  const DocumentModel = mongoose.model('documents');
  const betaDocument = await DocumentModel.getBetaDocumentContentBySource(
    source,
    sid,
  );
...
schema.methods.insertNoteMarkToContent = async function () {
  ...
  this.content = markNotes.setMark(
    this.content,
...
schema.statics.getStableDocumentRenderingContent = async (source, sid, uid) => {
  const DocumentModel = mongoose.model('documents');
  const stableDocument = await DocumentModel.getStableDocumentContentBySource(
    source,
    sid,
  );
  ...
  for (let text of texts) {
    ...
    for (let item of arr) {
      ...
      const targetUsers = await UserModel.find(
        { usernameLowerCase: { $in: usernames } },
        { username: 1, usernameLowerCase: 1, uid: 1 },
      );
      let user;
      // 取用户名最长的用户为目标用户
      ...
      if (atUsersId.includes(user.uid)) {
        continue;
      }
...
schema.methods.sendMessageToAtUsers = async function (from) {
  if (!['post', 'article'].includes(from)) {
    ThrowServerInternalError(`document from error`);
  }
  ...
  for (const user of atUsers) {
    // 跳过已经 AT 过的用户
    if (oldAtUsersId.includes(user.uid)) {
      continue;
    }
...
/*schema.statics.checkGlobalPostPermission = async (
  ...
  if (authLevel < authLevelMin) {
    ThrowCommonError(
      403,
      `身份认证等级未达要求，请至少完成身份认证 ${authLevelMin}`,
    );
  }
  // 获取今日发表数
  ...
  if (
    ...
    if (examNotPass.limited && examNotPass.count <= documentCountToday) {
      if (!examVolumeAD && !examVolumeA && !examVolumeB) {
        ...
        ThrowCommonError(
          403,
          `今日发表次数已达上限（${examNotPass.count} 次），请参加考试，通过后可获取更多发表权限`,
        );
      }
    }
  }
  ...
  if (intervalItem.limited) {
    ...
    if (latestDocument) {
      ThrowCommonError(
        403,
        `您当前的账号等级限定发表间隔时间不能小于 ${intervalItem.interval} 分钟，请稍候再试`,
      );
    }
  }
  if (countItem.limited && documentCountToday >= countItem.count) {
    ThrowCommonError(
      403,
...
schema.methods.initQuote = async function (quoteDid) {
  const DocumentModel = mongoose.model('documents');
  const document = await DocumentModel.findOnly({
    _id: quoteDid,
    type: 'stable',
  });
  if (!document) {
    ThrowCommonError(400, '未找到引用信息');
  }
  await this.updateOne({
    $set: {
      quoteDid,
 ...
 * */
schema.statics.getBetaDocumentsBySource = async (
  source,
  sourcesId,
  format = 'array',
) => {
  ...
  return await DocumentModel.getDocumentsBySource(
    beta,
    source,
    sourcesId,
    format,
 ...
 * @return {[Document] or Object}
 * */
schema.statics.getStableDocumentsBySource = async (
  source,
  sourcesId,
  format = 'array',
) => {
 ...
 * @return {[Document] or Object}
 * */
schema.statics.getDocumentsBySource = async (
  type,
  source,
  sourcesId,
  format = 'array',
...
schema.statics.getDocumentsUrlByDocumentsId = async (documentsId) => {
  ...
  const articlesObj = await ArticleModel.getArticlesObjectByArticlesId(
    articlesId,
  );
  const comments = await CommentModel.getCommentsObjectByCommentsId(commentsId);
  const commentsInfo = await CommentModel.getCommentsInfo(
...
schema.statics.insertSystemRecordContent = async (type, user, ctx) => {
  ...
  for (const enabledScore of enabledScores) {
    ...
    if (data.problem) {
      newRecords.problemId = data.problem._id;
    }
...
schema.statics.disabledToDraftDocuments = async function () {
  ...
  const { article: articleSource, comment: commentSource } =
    await DocumentModel.getDocumentSources();
  const { faulty: faultyStatus, disabled: disabledStatus } =
    await DocumentModel.getDocumentStatus();
  ...
  for (const log of onLog) {
    ...
    if (document && document.status === faultyStatus) {
      ...
      await DocumentModel.insertSystemRecordContent(
        `${document.source}Blocked`,
        user,
        {
          state: {},
          data: {
...
schema.statics.renderAtUsers = (content, atUsers = []) => {
  ...
  return content.replace(new RegExp(`@(${names.join('|')})`, 'ig'), (c, v) => {
    ...
    if (uid) {
      ...
      element('a').attr('href', getUrl('userHome', uid)).text(`@${v}`);
