...
threadSchema
  .virtual('threadCategoriesWarning')
  .get(function () {
...
threadSchema.methods.extendLastPost = async function () {
  ...
  return (this.lastPost = (
    await PostModel.find({ tid: this.tid, disabled: { $nin: [true] } })
      .sort({ toc: -1 })
      .limit(1)
  )[0]);
};
...
threadSchema.methods.extendCategory = async function () {
  ...
  return (this.category = await ThreadTypeModel.findOne({
    fid: this.fid,
    cid: this.cid,
  }));
};
...
threadSchema.methods.updateThreadVote = async function () {
  ...
  const voteUpMax = await PostModel.findOne({ tid: this.tid }).sort({
    voteUp: -1,
  });
...
threadSchema.methods.updateThreadEncourage = async function () {
  ...
  const encourageTotal = await KcbsRecordModel.countDocuments({
    type: 'creditKcb',
    pid: { $in: pid },
  });
...
threadSchema.methods.updateThreadMessage = async function (toSearch = true) {
  ...
  updateObj.countToday = await PostModel.countDocuments({
    tid: thread.tid,
    type: 'post',
    toc: { $gt: today },
    parentPostId: '',
  });
  updateObj.countRemain = await PostModel.countDocuments({
    tid: thread.tid,
    type: 'post',
    disabled: { $ne: true },
    parentPostId: '',
...
threadSchema.methods.newPost = async function (post, user, ip) {
  ...
  let quotePost;
  if (quote) {
    quotePost = await PostModel.findOne(
      { tid: this.tid, pid: post.quote, type: 'post' },
      { cv: 1, pid: 1, uid: 1 },
    );
...
threadSchema.statics.extendThreads = async (threads, options) => {
  ...
  threads.map((thread) => {
    ...
    if (o.lastPost && thread.lm) {
      postsId.add(thread.lm);
    }
  ...
  if (o.category) {
    const categories = await ThreadTypeModel.find({
      cid: { $in: [...new Set(cid)] },
    });
  ...
  for (const thread of threads) {
    ...
    if (o.firstPost) {
      ...
      if (firstPost.anonymous && o.excludeAnonymousPost) {
        continue;
      }
      if (o.firstPostUser) {
        let user;
    ...
    if (o.category) {
      if (thread.categoriesId && thread.categoriesId.length !== 0) {
        for (const cid of thread.categoriesId) {
          if (categoryObj[cid]) {
            thread.categories.push(categoryObj[cid]);
          }
        }
      }
    }
...
threadSchema.statics.ensurePublishPermission = async (options) => {
  ...
  await Promise.all(
    forums.map(async (forum) => {
      if (await SettingModel.isRecycle(forum.fid)) {
        throwErr(400, '不允许在回收站专业发表内容');
      }
      const childrenForums = await forum.extendChildrenForums();
      if (childrenForums.length !== 0) {
        throwErr(
          400,
          `专业【${forum.displayName}】下存在其他专业，请到下属属专业发表内容。`,
        );
      }
    }),
  );
  ...
  if (contentLength(title) > 200) {
    throwErr(400, '标题不能大于200字节');
  }
  if (!content) {
    throwErr(400, '内容不能为空');
  }
  if (contentLength(content) < 6) {
    throwErr(400, '内容不能小于6个字节');
  }
  ...
  if ((!volumeB || !user.volumeB) && (!volumeA || !user.volumeA)) {
    ...
    if (!unlimited && countLimit <= todayThreadCount) {
      throwErr(403, '今日发表文章次数已用完，请明天再试。');
    }
  }
  // 发表回复时间、条数限制
...
threadSchema.methods.isModerator = async function (uid, type) {
  ...
  if (type === 'or') {
    for (const forum of forums) {
      if (await forum.isModerator(user)) {
        return true;
      }
    }
    return false;
...
threadSchema.statics.updateAutomaticRecommendThreadsByType = async (type) => {
  ...
  let exception = [];
  if (type === 'fixed') {
    exception = movable.manuallySelectedThreads.concat(
      movable.automaticallySelectedThreads,
      fixed.manuallySelectedThreads,
    );
  } else {
    exception = fixed.manuallySelectedThreads.concat(
 ...
 * @author pengxiguaa 2020/7/15
 * */
threadSchema.statics.updateHomeRecommendThreadsByType = async (
  type,
  excludedThreadsId = [],
) => {
  const ThreadModel = mongoose.model('threads');
  ...
  const {
    ...
    digest,
    postVoteUpMinCount,
    postVoteDownMaxCount,
    otherThreads,
    threadVoteUpMinCount,
    reportedAndUnReviewed,
    original,
    flowControl,
...
threadSchema.statics.getHomeRecommendThreadsByType = async (type, fid = []) => {
  if (!['fixed', 'movable'].includes(type)) {
    throwErr(500, `推荐文章类型错误 type: ${type}`);
  }
  const SettingModel = mongoose.model('settings');
  const apiFunction = require('../nkcModules/apiFunction');
  const homeSettings = await SettingModel.getSettings('home');
  ...
  let {
    displayType,
    order,
    manuallySelectedThreads,
    automaticallySelectedThreads,
    automaticProportion,
    count,
  } = options;
  let threadsId = manuallySelectedThreads.concat(automaticallySelectedThreads);
  threadsId = threadsId.map((t) => t.tid);
  const threads = await ThreadModel.find(
    { tid: { $in: threadsId }, mainForumsId: { $in: fid } },
    { tid: 1 },
  );
  const threadsObj = {};
  threads.map((thread) => {
  ...
  manuallySelectedThreads = manuallySelectedThreads.filter(
    (t) => threadsObj[t.tid],
  );
  automaticallySelectedThreads = automaticallySelectedThreads.filter(
    (t) => threadsObj[t.tid],
  );
  let results = [];
  if (displayType === 'manual') {
    // 只从手动推荐文章中选取
    if (order === 'random') {
      manuallySelectedThreads = apiFunction.arrayShuffle(
        manuallySelectedThreads,
      );
    }
    ...
    // 只从自动推荐文章中选取
    if (order === 'random') {
      automaticallySelectedThreads = apiFunction.arrayShuffle(
        automaticallySelectedThreads,
      );
    }
    ...
    results = manuallySelectedThreads
      .slice(0, manualCount)
      .concat(automaticallySelectedThreads.slice(0, automaticCount));
    if (order === 'random') {
...
threadSchema.statics.extendFundInfo = async function (props) {
  const {
    ...
    uid,
    accessForumsId,
    displayFundApplicationFormSecretInfoPermission = false,
  } = props;
  ...
  const targetUserInFundBlacklist = await FundBlacklistModel.inBlacklist(
    applicationForm.uid,
  );
  await applicationForm.hideApplicationFormInfoByUserId(
    uid,
    displayFundApplicationFormSecretInfoPermission,
  );
