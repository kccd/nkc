...
const forumSchema = new Schema({
    ...
    threadListStyle: {
        type: {
            // 显示样式 abstract: 摘要模式, brief: 简略模式, minimalist: 极简模式
            type: String,
            default: 'abstract',
        },
        cover: {
            // 封面图位置 left: 左侧, right: 右侧, null: 不显示封面图
            type: String,
            default: 'left',
        },
    },
...
forumSchema.statics.updateCount = async function(threads, isAdd) {
    ...
    const updateParentForums = async(forum, tif) => {
        ...
        const countThreads = isAdd ?
            forum.countThreads + tif.length :
            forum.countThreads - tif.length;
        ...
        await updateParentForums(
            await ForumModel.findOne({ fid: forum.parentsId }),
            tif,
        );
    };
...
forumSchema.methods.newPost = async function(
    post,
    user,
    ip,
    cids,
    ...
    await thread.updateOne({
        $set: { lm: _post.pid, oc: _post.pid, count: 1, hits: 1 },
    });
...
forumSchema.methods.getBreadcrumbForums = async function() {
    ...
    if (fid.length === 0 || !this.parentsId.length) {
        return fid;
    }
...
// ----------------------------- 加载能管理的板块fid ---------------------------------
forumSchema.statics.canManagerFid = async function(
    roles,
    grade,
    user,
    baseFid,
    ...
    const fid = await ForumModel.getAccessibleForumsId(
        roles,
        grade,
        user,
        baseFid,
...
// ----------------------------- 加载能管理的板块 ---------------------------------
forumSchema.statics.canManagerForums = async function(
    roles,
    grade,
    user,
    baseFid,
...
forumSchema.statics.visibleFid = async(roles, grade, user, baseFid) => {
    ...
    const accessibleFid = await ForumModel.getAccessibleForumsId(
        roles,
        grade,
        user,
        baseFid,
    ...
    const canNotDisplayOnNavForumsId = await client.smembersAsync(
        'canNotDisplayOnNavForumsId',
    );
    // 无权用户也能看到入口的专业ID
    const canDisplayOnNavForumsIdNCC = await client.smembersAsync(
        'canDisplayOnNavForumsIdNCC',
    );
...
forumSchema.methods.ensureForumPermission = function(options) {
    ...
    if (uid && this.moderators.includes(uid)) {
        return;
    }
    let hasRole = false;
    ...
    if (this.relation === 'and') {
        ...
        const err = new Error(
            `专业数据错误(fid: ${this.fid}, relation: ${this.relation})`,
        );
        err.status = 500;
        throw err;
    }
...
forumSchema.statics.getAllChildrenForums = async function(fid) {
    ...
    const findForums = async(parentId) => {
        const forums = await ForumModel.find({ parentsId: parentId }).sort({
            order: 1,
        });
...
forumSchema.statics.ensureForumsPermission = async(arr, userInfo) => {
    ...
    const UserModel = mongoose.model('users');
    if (!arr) {
        throwErr(500, 'arr is a required parameter in forum.statics method');
    }
    let forums = arr;
    if (forums.length === 0) {
        return;
    ...
    if (!userInfo) {
        throwErr(500, `userInfo is a required parameter in forum.statics method`);
    }
    let user = userInfo;
    ...
    await Promise.all(
        forums.map(async(forum) => {
            if (fid.includes(forum.fid)) {
                return;
            }
            throwErr(
                403,
                `您没有权限访问专业【${forum.displayName}】，且无法在该专业下发表任何内容。`,
            );
        }),
    );
};
...
forumSchema.statics.getThreadForumsId = async(roles, grade, user, baseFid) => {
    ...
    if (baseFid) {
        ...
        for (const relatedFid of forum.relatedForumsId) {
            const rFid = await ForumModel.getAccessibleForumsId(
                roles,
                grade,
                user,
                relatedFid,
...
// 获取有权限访问的专业
forumSchema.statics.getAccessibleForums = async(
    roles,
    grade,
    user,
    baseFid,
...
forumSchema.statics.getAllShopForums = async(roles, grade, user, baseFid) => {
    ...
    return await ForumModel.find({ fid: { $in: fid }, kindName: 'shop' }).sort({
        order: 1,
    });
};

// 获取有权限访问的专业ID
forumSchema.statics.getAccessibleForumsId = async(
    roles,
    grade,
    user,
    baseFid,
    ...
    if (!user) {
        ...
        for (const roleId of rolesId) {
            const fidForRoleAndGrade = await client.smembersAsync(
                `role-grade:${roleId}-${gradeId}:forumsId`,
            );
    ...
    if (baseFid) {
        const childFid = await client.smembersAsync(
            `forum:${baseFid}:allChildForumsId`,
        );
...
forumSchema.methods.extendRelatedForums = async function(fids) {
    ...
    const relatedForums = await ForumModel.find({
        fid: { $in: relatedForumsId },
    });
 ...
 * */
forumSchema.statics.getForumsTreeLevel2 = async(
    userRoles,
    userGrade,
    user,
) => {
    ...
    for (const forum of forums) {
        ...
        if (!forum.childrenForums.length) {
            forum.latestThreads = await ForumModel.getLatestThreadsFromRedis(
                forum.fid,
            );
        } else {
            forum.latestThreads = [];
...
forumSchema.statics.createNewThread = async function(options) {
    if (!options.uid) {
        throwErr(400, 'uid不可为空');
    }
    if (!options.fids || options.fids.length === 0) {
        throwErr(400, '目标专业fids不可为空');
    }
    const SettingModel = mongoose.model('settings');
    const SubscribeModel = mongoose.model('subscribes');
    const ThreadModel = mongoose.model('threads');
    ...
    await thread.updateOne({
        $set: { lm: _post.pid, oc: _post.pid, count: 0, hits: 0 },
    });
...
forumSchema.methods.createLibrary = async function(uid) {
    if (this.lid) {
        throwErr(500, '专业文库已存在，fid: ${this.fid}, lid: ${this.lid}');
    }
...
forumSchema.statics.getRecommendForums = async(fid = []) => {
    ...
    const forums = await mongoose
        .model('forums')
        .find({ fid: { $in: recommendForumsId.filter((f) => fid.includes(f)) } });
...
forumSchema.methods.saveLatestThreadToRedis = async function(count = 3) {
    ...
    for (const thread of threads) {
        const { tid, tlm, oc, uid, hits, count, toc, digest, lm } = thread;
...
forumSchema.statics.saveAllForumsIdToRedis = async(forums) => {
    ...
    await client.setArray(
        getRedisKeys('isVisibilityNCCForumsId'),
        isVisibilityNCCForumsId,
    );
    await client.setArray(
        getRedisKeys('displayOnParentForumsId'),
        displayOnParentForumsId,
    );
    await client.setArray(
        getRedisKeys('displayOnSearchForumsId'),
        displayOnSearchForumsId,
    );
};
/*
...
forumSchema.statics.checkPermissionCore = async(type, user, fid = []) => {
    ...
    for (const id of fid) {
        ...
        // 发表文章，只允许发表到最底层专业
        if (type === 'write') {
            const childForumsId = await ForumModel.getAllChildForumsIdByFid(
                forum.fid,
            );
        ...
        let hasRole = false,
            hasGrade = userGradeId !== null && gradesId.includes(userGradeId);
...
forumSchema.statics.checkPublishNotice = async({ uid, id, type }) => {
    ...
    //编辑文章
    if (type === 'thread') {
        const thread = await ThreadModel.findOnly({ oc: id }, { uid: 1, mainForumsId: 1 }, );
        //判断是否为作者本人
        if (thread.uid !== uid) {
    ...
    //编辑回复
        ...
        const post = await PostModel.findOnly({ pid: id }, { mainForumsId: 1, tid: 1 }, );
...
forumSchema.statics.checkEditPostPosition = async({ uid, tid, isAdmin }) => {
    const ForumModel = mongoose.model('forums');
    const ThreadModel = mongoose.model('threads');
    const thread = await ThreadModel.findOnly({ tid }, { uid: 1, mainForumsId: 1 }, );
    //判断是否有管理员权限可以使用该功能
    if (isAdmin) {
        return { status: 200 };
    }
...
forumSchema.statics.checkGlobalPostPermission = async(uid, type) => {
    ...
    let { authLevelMin, exam } = postSettings[settingsType.key];
...
forumSchema.methods.getForumNav = async function(cid) {
    ...
    if (cid) {
        const category = await ThreadTypeModel.findOne({
            fid: this.fid,
            cid: Number(cid),
        });
        if (category) {
...
forumSchema.statics.checkForumCategoryBeforePost = async(_fids) => {
    ...
    if (minorForumId.length < min || minorForumId.length > max) {
        throwErr(400, `专业分类辅分类数量不符合要求`);
    }
    // 以下检测专业分类冲突
    ...
    for (let i = 0; i < _categories.length; i++) {
        for (let j = 0; j < _categories.length; j++) {
            ...
            if (i === j || !c.excludedCategoriesId.includes(cc._id)) {
                continue;
            }
            if (c._id === cc._id) {
                ...
                throwErr(
                    400,
                    `不允许同时选择「${c.name}」与「${cc.name}」下的专业，请重新选择`,
                );
            }
        }
    }
...
forumSchema.statics.checkAccessControlPermissionWithThrowError = async(
    props,
) => {
    const { uid, rolesId, gradeId, isApp } = props;
    const AccessControlModel = mongoose.model('accessControl');
...
forumSchema.statics.getUserCategoriesWithForums = async(props) => {
    const ForumModel = mongoose.model('forums');
    const ForumCategoryModel = mongoose.model('forumCategories');
    const { user, userRoles, userGrade, limitLevel = false } = props;
    let forumsTree;
    if (limitLevel) {
