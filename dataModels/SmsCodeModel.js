...
smsCodeSchema.statics.ensureCode = async (props) => {
  ...
  if (!smsCode) {
    ThrowCommonError(400, '短信验证码无效');
  } else if (smsCode.ip !== ip) {
    ThrowCommonError(400, `验证码无效`);
  } else if (smsCode.toc < Date.now() - setting.validityPeriod * 60 * 1000) {
    ThrowCommonError(400, `验证码已过期`);
  }
  return smsCode;
};
...
smsCodeSchema.statics.ensureSendPermission = async (obj) => {
  ...
  let smsCodes = await SmsCodeModel.find({
    nationCode,
    mobile,
    type,
    toc: { $gte: today },
  ...
  if (smsCodes.length !== 0) {
    const timeLimit = smsCodes[0].toc - (Date.now() - smsInterval * 1000);

    if (timeLimit > 0) {
      // const err = new Error(`发送验证码的间隔不能小于120秒，请${(timeLimit/1000).toFixed(0)}秒后再试`);
      const err = new Error(`发送验证码的间隔不能小于${smsInterval}秒`);
      err.status = 400;
      throw err;
    }
  }
  ...
  if (smsCodes.length >= setting.sameMobileOneDay) {
    const err = new Error(
      `每天发送相同类型的验证码条数不能超过${setting.sameMobileOneDay}条。`,
    );
    err.status = 400;
    throw err;
  }
  ...
  if (smsCodes.length >= setting.sameIpOneDay) {
    const err = new Error(
      `同IP每天发送相同类型的验证码条数不能超过${setting.sameIpOneDay}条。`,
    );
    err.status = 400;
    throw err;
  }
