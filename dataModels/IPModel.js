...
schema.statics.getIPInfo = async (type, value) => {
  const IPModel = mongoose.model('ips');
  if (!['ip', '_id'].includes(type)) {
    throwErr(500, `getIPInfo type类型错误`);
  }
  const match = {};
  match[type] = value;
  ...
  if (!ipData) {
    if (type === '_id') {
      throwErr(400, `未找到${value}对应的IP地址`);
    }
  ...
  if (
    ipData.adCode &&
    ipData.tlm &&
    ipData.tlm > Date.now() - 180 * 24 * 60 * 60 * 1000
  ) {
    return {
      ip: ipData.ip,
      token: ipData._id,
  ...
  let { province, city, adcode, rectangle } = await apiFunction.getIpAddress(
    ipData.ip,
  );
...
schema.statics.getIPInfoFromLocal = async (ip, hideServiceProvider = false) => {
  ...
  let location = '';
  try {
    const { country, province, city, isp } =
...
schema.statics.getIpsAddrInfoById = async (ids) => {
  const IPModel = mongoose.model('ips');
  const ipsData = await IPModel.find(
    { _id: { $in: ids } },
    {
      ip: 1,
      _id: 1,
...
schema.statics.getIpsAddrInfo = async (ips) => {
  ...
  for (let ip of ips) {
    ...
    let addr = '';
    try {
      const { country, province } = await apiFunction.getIpInfoFromLocalModule(
        ip,
      );
      if (country) {
...
schema.statics.getIpCity = async (ip) => {
  const apiFunction = require('../nkcModules/apiFunction');
  ip = ip || '0.0.0.0';
  let result = '';
  try {
