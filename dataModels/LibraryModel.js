...
schema.statics.newFolder = async (options = {}) => {
  ...
  if (!userCount) {
    throwErr(500, `user not found. uid: ${uid}`);
  }
  let pl;
  if (lid) {
    pl = await LibraryModel.findOne({ _id: lid });
    if (!pl) {
      throwErr(500, `library not found. lid: ${lid}`);
    }
    const nav = await pl.getNav();
    // 文件夹层数不能超过10
    if (nav.length >= 10) {
      throwErr(400, `文件夹层数超出限制`);
    }
  }
...
schema.statics.newFile = async (options = {}) => {
  const { name, description, category, lid, resource, ip, port, user } =
    options;
  const LibraryModel = mongoose.model('libraries');
  const SettingModel = mongoose.model('settings');
  const type = 'file';
  ...
  const userCount = await mongoose.model('users').countDocuments({ uid });
  if (!userCount) {
    throwErr(500, `资源uid异常，rid: ${rid}, uid: ${uid}`);
  }
  const pl = await LibraryModel.findOne({ _id: lid });
  if (!pl) {
    throwErr(500, `文件夹id异常，lid: ${lid}`);
  }
  const library = LibraryModel({
    _id: await SettingModel.operateSystemID('libraries', 1),
    type,
...
schema.statics.checkLibraryInfo = async (type, options) => {
  ...
  if (saveName !== 0) {
    throwErr(400, `当前目录下${valueName}名已存在`);
  }
};
...
schema.methods.getFolders = async function () {
  ...
  let folders = await LibraryModel.find({
    lid: _id,
    type: 'folder',
    deleted: false,
  });
...
schema.methods.getFiles = async function () {
  ...
  const LibraryModel = mongoose.model('libraries');
  const pinyin = require('../nkcModules/pinyin.js');
  let files = await LibraryModel.find({
    lid: _id,
    type: 'file',
    deleted: false,
  });
...
schema.methods.computeCount = async function () {
  ...
  nav.reverse();
  for (const n of nav) {
    const folders = await LibraryModel.find(
      { lid: n._id, type: 'folder', deleted: false },
      { fileCount: 1, folderCount: 1 },
    );
    let fileCount = await LibraryModel.countDocuments({
      lid: n._id,
      type: 'file',
      deleted: false,
    });
...
schema.statics.getPermission = async (user) => {
  ...
  let operations = [];
  const librarySettings = await mongoose
    .model('settings')
    .getSettings('library');
  ...
  if (pg) {
    operations = operations.concat(pg.operations);
  }
  return [...new Set(operations)];
};

schema.methods.ensurePermission = async function (
  user,
  operation,
  modifyOtherLibrary,
) {
  if (modifyOtherLibrary) {
    return;
  }
  if (
    ['createFile', 'createFolder'].includes(operation) ||
    this.uid === user.uid
  ) {
...
schema.statics.saveToES = async (lid) => {
  ...
  if (!library) {
    throwErr(500, `文件不存在，lid: ${lid}`);
  }
  if (library.deleted) {
    throwErr(500, `文件已被标记为删除，无法同步到搜索数据库。`);
  }
...
schema.statics.saveAllLibraryFileToElasticSearch = async () => {
  ...
  for (let i = 0; i <= count; i += limit) {
    const files = await LibraryModel.find(match, { _id: 1 })
      .sort({ toc: 1 })
      .skip(i)
      .limit(limit);
    ...
    console.log(
      `【同步LibraryFile到ES】 总：${count}, 当前：${i} - ${i + limit}`,
    );
  }
 ...
 * */
schema.statics.saveAllLibraryFileToElasticSearchBatch = async () => {
  const LibraryModel = mongoose.model('libraries');
  const elasticSearch = require('../nkcModules/elasticSearch');
  const match = {
    type: 'file',
  ...
  console.log(`开始批量同步 ${count} 个文库文件到ElasticSearch...`);

  for (let i = 0; i < count; i += batchSize) {
    const libraries = await LibraryModel.find(match)
      .sort({ toc: 1 })
      .skip(i)
      .limit(batchSize);

    const documents = libraries.map((library) => ({
      docType: 'resource',
      document: library,
    }));
    ...
    console.log(
      `【批量同步LibraryFile到ES】 总：${count}, 已完成：${
        i + libraries.length
      }`,
    );
  }

  console.log('【批量同步LibraryFile到ES】完成');
};
