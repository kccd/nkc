...
schema.statics.getTimeline = async (columnId) => {
  ...
  const firstColumnPost = await ColumnPostModel.findOne({}, { top: 1 }).sort({
    top: 1,
  });
  if (!firstColumnPost) {
    return [];
...
schema.statics.saveColumnToElasticSearch = async (column) => {
  if (!column) {
    ThrowServerInternalError('专栏不能为空');
  }
  const data = {
...
schema.statics.extendColumnsPosts = async (columns, count) => {
  ...
  for (let column of columns) {
    ...
    for (const cp of columnPosts) {
      ...
      tocObj[pid] = toc;
      if (!columnIdObj[columnId]) {
        columnIdObj[columnId] = [];
      }
...
schema.methods.updateBasicInfo = async function () {
  ...
  const lastPost = await ColumnPostModel.findOne(
    { columnId: _id },
    { toc: 1 },
  ).sort({ toc: -1 });
...
schema.statics.updateHomeHotColumns = async () => {
  ...
  const { columnCount, minPostCount, updateTime, minSubscriptionCount } =
    homeSettings.columnPool;
  const existingColumnsId = homeSettings.columnsId.concat(
...
schema.statics.clearColumnName = async (columnId) => {
  const ColumnModel = mongoose.model('columns');
  const SettingModel = mongoose.model('settings');
  const serverSettings = await SettingModel.getSettings('server');
  const newName = `${serverSettings.websiteCode}-m-${columnId}`;
  const newNameLowerCase = newName.toLowerCase();
  await ColumnModel.updateOne(
    { _id: columnId },
    { $set: { name: newName, nameLowerCase: newNameLowerCase } },
  );
};

schema.statics.clearColumAbbr = async (columnId) => {
  const ColumnModel = mongoose.model('columns');
