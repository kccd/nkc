...
import Vue from 'vue';
import { sweetPrompt, sweetError } from '../../../lib/js/sweetAlert';
import { visitUrl } from '../../../lib/js/pageSwitch';
import { nkcAPI } from '../../../lib/js/netAPI';
import CommonModal from '../../../lib/vue/CommonModal.vue';
const { applicationForm, type } = getDataById('data');
...
window.app = new Vue({
  ...
  methods: {
    ...
    setReasonInfo(type, props) {
      $(`div[data-type="${type}"] textarea`).val(props.reason);
      const targetInput = $(`div[data-type="${type}"] input`);
      targetInput.eq(0).prop('checked', props.passed);
      targetInput.eq(1).prop('checked', !props.passed);
    },
    getTitleByType(type) {
      let title = '';
      if (type === 'userInfoAudit') {
        title = '审核人信息';
      } else if (type === 'projectAudit') {
        title = '项目信息';
      } else {
        title = '资金预算';
      }
    ...
    openReasonInput(type) {
      let title = this.getTitleByType(type);
      const reasonObj = this.getReasonInfo(type);
      this.$refs.commonModal.open(
        (data) => {
          this.setReasonInfo(type, {
            reason: data[0].value,
            passed: data[1].value,
          });
    ...
    refuse() {
      ...
      return sweetPrompt(`请输入拒绝的理由`, this.refuseReason)
        .then((reason) => {
          ...
          return nkcAPI(
            `/fund/a/${self.applicationForm._id}/manage/refuse`,
            'POST',
            {
              reason,
            },
        ...
        .then(() => {
          visitUrl(`/fund/a/${self.applicationForm._id}`);
        })
        .catch((err) => {
          sweetError(err);
        });
      ...
      return Promise.resolve()
        .then(() => {
          if (self.type === 'project') {
            ...
            return nkcAPI(
              `/fund/a/${self.applicationForm._id}/manage/audit/project`,
              'POST',
              {
                comments,
                suggest,
            ...
            return nkcAPI(
              `/fund/a/${self.applicationForm._id}/manage/audit/info`,
              'POST',
              {
                comment,
                fact,
