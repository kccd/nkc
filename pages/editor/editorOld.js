  ...
  window.PostInfo = new Vue({
    ...
    mounted: function () {
      ...
      if (self.oldDraft) {
        ...
        let info =
          '是否加载 ' +
          self.format('YYYY/MM/DD HH:ss:mm', self.oldDraft.tlm) +
          ' 编辑但未提交的内容？';
        sweetQuestion(info)
          .then(function () {
            // 用户选择了加载，则跳转到相应的草稿页面
            if (NKC.configs.isApp) {
              window.location.href =
                '/editor?type=redit&id=' + self.oldDraft.did;
            } else {
              self.visitUrl('/editor?type=redit&id=' + self.oldDraft.did);
            }
          })
          .catch(function () {
            // 用户选择了取消加载，则启动自动保存草稿且开启关闭页面的警告
    ...
    methods: {
      ...
      selectCover: function () {
        ...
        // }
        self.$refs.resourceSelector.open(
          function (data) {
      ...
      getThreadCategoriesId() {
        const tcId = [];
        for (const tc of this.threadCategories) {
          if ([null, 'default'].includes(tc.selectedNode)) {
            continue;
          }
        ...
        ForumSelector.open(
          function (r) {
            ...
            if (type === 'mainForum') {
              ...
              if (self.selectedForumsId.indexOf(r.fid) !== -1) {
                return;
              }
      ...
      checkForums: function () {
        if (!this.selectedForumsId.length) {
          throw '请至少选择一个专业';
        }
      ...
      getPost: function () {
        ...
        const postButton = getPostButton();
        if (
          postButton.havePermissionToSendAnonymousPost &&
          postButton.allowedAnonymous
        ) {
        ...
        if (PostToColumn && PostToColumn.getStatus) {
          ...
          if (status.checkbox) {
            ...
            post.columnMinorCategoriesId =
              status.selectedMinorCategoriesId || [];
          }
        }
        // 调查表单数据
        if (PostSurvey) {
      ...
      disablePostButton() {
        if (window.PostButton) {
          window.PostButton.disabledSubmit = true;
        }
        if (window.FloatPostButton) {
          window.FloatPostButton.disabledSubmit = true;
        }
      },
      enablePostButton() {
        if (window.PostButton) {
          window.PostButton.disabledSubmit = false;
        }
        if (window.FloatPostButton) {
          window.FloatPostButton.disabledSubmit = false;
        }
      },
      // 提交内容
      submit: function () {
          ...
          .then(function () {
            if (type === 'newThread') {
              ...
              if (self.coverData) {
                formData.append(
                  'postCover',
                  NKC.methods.blobToFile(self.coverData),
                  'cover.png',
                );
              }
...
function mediaInsertUE(rid, fileType, name, mediaType) {
  ...
  NKC.methods.appResourceToHtml(resource).then(function (html) {
    try {
