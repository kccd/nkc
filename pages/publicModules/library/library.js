    ...
    self.app = new Vue({
      ...
      mounted() {
        ...
        if (typeof queryId !== 'number') {
          const libraryVisitFolderLogs = NKC.methods.getFromLocalStorage(
            'libraryVisitFolderLogs',
          );
      ...
      methods: {
        ...
        addFileByRid() {
          ...
          if (!uploadResourcesId || uploadResourcesId.length <= 0) {
            return;
          }
        ...
        uploadFile(index, arr) {
          ...
          Promise.resolve()
            .then(() => {
              ...
              if (
                !['media', 'paper', 'book', 'program', 'other'].includes(
                  file.category,
                )
              ) {
                throw '未选择文件分类';
            ...
            .then(() => {
              if (file.type === 'modify') {
                ...
                // 将线上文件提交到文库
                const { name, description, category, rid, folder } = file;
                const body = {
                  rid,
                  name,
                  description,
        ...
        back() {
          if (this.lastFolder) {
            this.selectFolder(this.lastFolder);
          }
        },
        // 切换到文件上传
        toUpload() {
        ...
        saveCategoriesToLocalStorage() {
          ...
          const libraryListCategories = NKC.methods.getFromLocalStorage(
            'libraryListCategories',
          );
          libraryListCategories[this.lid] = listCategories;
          NKC.methods.saveToLocalStorage(
            'libraryListCategories',
            libraryListCategories,
          );
        },
        ...
        addHistory(lid) {
          ...
          if (this.lastHistoryLid && this.lastHistoryLid === lid) {
            return;
          }
              ...
              nkcAPI(url, method, body)
                .then((data) => {
                  sweetSuccess(
                    `执行成功${
                      data.ignoreCount
                        ? `，共有${data.ignoreCount}个项目因存在冲突或不是你自己发布的而被忽略`
                        : ''
                    }`,
                  );
                  self.app.mark = false;
        ...
        deleteFolder(foldersId) {
          ...
          sweetQuestion(`确定要执行删除操作？`)
            .then(function () {
              nkcAPI(
                `/library/${self.app.folder._id}/list?lid=${foldersId}`,
                'DELETE',
              )
                .then(function (data) {
                  self.app.mark = false;
                  ...
                  sweetSuccess(
                    `执行成功${
                      data.ignoreCount
                        ? `，共有${data.ignoreCount}个项目因不是你自己发布的而被忽略`
                        : ''
                    }`,
                  );
                })
