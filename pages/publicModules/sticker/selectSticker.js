  ...
  self.app = new Vue({
    ...
    methods: {
      ...
      initModule() {
        ...
        self.dom.draggable({
          ...
          drag: function (event, ui) {
            ...
            if (ui.position.top > height - 30) {
              ui.position.top = height - 30;
            }
            var width = self.dom.width();
            if (ui.position.left < 100 - width) {
              ui.position.left = 100 - width;
            }
            var winWidth = $(window).width();
            if (ui.position.left > winWidth - 100) {
              ui.position.left = winWidth - 100;
            }
          },
        });
      },
      ...
      changePage(type) {
        ...
        if (paging.buttonValue.length <= 1) {
          return;
        }
        if (type === 'last' && paging.page === 0) {
          return;
        }
        if (type === 'next' && paging.page + 1 === paging.pageCount) {
          return;
        }
      ...
      fastSelectPage() {
        ...
        const paging = this.paging;
        if (!paging || !paging.buttonValue.length) {
          return;
        }
        const lastNumber =
          paging.buttonValue[paging.buttonValue.length - 1].num;
      ...
      // 处理后的图片发送
      addLocalFile(file) {
        this.fileToSticker(file).then((sticker) => {
      ...
      uploadLocalSticker(sticker) {
        Promise.resolve()
          ...
          .then((data) => {
            ...
            this.stickers = [
              data.r,
              ...this.stickers.slice(0, this.stickers.length - 1),
            ];
            self.app.localStickers.splice(
              self.app.localStickers.indexOf(sticker),
              1,
            );
            if (!self.app.localStickers.length) {
              self.app.selectType('own');
            }
          })
          .catch((data) => {
