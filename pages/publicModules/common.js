...
var NKC = {
  ...
  on: function (eventName, callback) {
    if (!NKC.events[eventName]) {
      NKC.events[eventName] = [];
    }
  ...
  oneAfter: function (eventName, callback) {
    if (NKC.eventsLog[eventName]) {
      return callback(undefined, function () {});
    }
...
NKC.methods.markDom = function (dom) {
  ...
  var colorTimeout = setInterval(function () {
    ...
    dom.css(
      'background-color',
      'rgba(255, 251, 221, ' + (colorValue < 1 ? colorValue : 1) + ')',
    );
  }, 1000);
};
/*
...
NKC.methods.isMobilePhoneBrowser = function () {
  ...
  return (
    ...
    NKC.configs.platform !== 'reactNative'
  );
};
/*
 * 判断是否为电脑浏览器
