...
(function () {
  ...
  laydate.render({
    ...
    value:
      sTime && eTime
        ? dateFormat('yyyy-M-d H:m:s', new Date(parseInt(sTime))) +
          ' - ' +
          dateFormat('yyyy-M-d H:m:s', new Date(parseInt(eTime)))
        : '',
    type: 'datetime',
    range: true,
    done: function (date) {
  ...
  $('#search').on('click', function () {
    ...
    NKC.methods.visitUrl(
      '/e/log/smscode?c=' + NKC.methods.strToBase64(JSON.stringify(c)),
    );
  });
})();
