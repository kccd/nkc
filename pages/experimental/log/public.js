...
if ($cleanupTimeRange[0]) {
  ...
  laydate.render({
    ...
    format: 'yyyy-M-d H:m:s',
    value:
      (sTime ? sTime : dateFormat('yyyy-M-d', threeMonthAgo) + ' 0:0:0') +
      ' - ' +
      (eTime ? eTime : dateFormat('yyyy-M-d', threeMonthAgo) + ' 23:59:59'),
    type: 'datetime',
    range: true,
...
function deleteCurrentRecord() {
  ...
  return sweetQuestion('确认删除当前查询结果中的所有日志吗？')
    .then(function () {
      return nkcAPI(
        url + '?del=' + NKC.methods.strToBase64(JSON.stringify(c)),
        'DELETE',
      );
    })
    .then(function () {
