...
const auth3Codes = getDataById('auth3Codes').codes;
...
authenticate.card.expiryDate = `${expiryDate1.getFullYear()}年${
  expiryDate1.getMonth() + 1
}月${expiryDate1.getDate()}日`;
authenticate.video.expiryDate = `${expiryDate2.getFullYear()}年${
  expiryDate2.getMonth() + 1
}月${expiryDate2.getDate()}日`;

new Vue({
  el: '#app',
  data: {
    auth3Codes: auth3Codes,
  ...
  computed: {
    verifyDescription() {
      ...
      return auth3Content.replace(/{code}/gi, code);
    },
    IDCardA() {
