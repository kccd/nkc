...
var app = new Vue({
  ...
  methods: {
    ...
    selectForum: function (type) {
      ...
      const forums =
        type === 'selected' ? this.selectedForums : this.excludedForums;
      if (!window.ForumSelector) {
        window.ForumSelector = new NKC.modules.ForumSelector();
      }
      window.ForumSelector.open(
        function (data) {
          if (forumsId.includes(data.forum.fid)) {
            return;
          }
    ...
    search: function () {
      if (!this.c) {
        return screenTopWarning('请输入关键词');
      }
      if (this.selectedForumsId.length > 0) {
        const forumsId = this.selectedForumsId.filter(
          (fid) => !this.excludedForumsId.includes(fid),
        );
  ...
  mounted: function () {
    ...
    if (options) {
      try {
        ...
        if (options.excludedFid) {
          this.excludedFid = options.excludedFid;
        }
        if (options.onlyTitle) {
          this.onlyTitle = options.onlyTitle;
        }
      } catch (err) {}
    }
