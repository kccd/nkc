...
function toggleAnonymousDom(havePermission) {
  ...
  if (!anonymousDom.length || !checkbox.length) {
    return;
  }
  if (havePermission) {
...
function onPost() {
  ...
  if (queryType === 'post') {
    ...
    url = '/t/' + queryId;
    data = { post: post };
  } else if (queryType === 'application' && queryCat === 'p') {
    // 编辑项目内容
    method = 'PUT';
    url = '/fund/a/' + queryId;
    data = { project: post, s: 3 };
  } else if (queryType === 'application' && queryCat === 'c') {
    // 评论
    method = 'POST';
    url = '/fund/a/' + queryId + '/comment';
    data = { comment: post };
  } else if (queryType === 'application' && queryCat === 'r') {
    // 报告
    method = 'POST';
...
if (type == 'post') {
  ...
  disnoneplayHtml = disnoneplayHtml.replace(
    /<blockquote cite.+?blockquote>/gim,
    '',
  );
  ue.ready(function () {
...
function mediaInsertUE(srcStr, fileType, name) {
  ...
  if (
    ...
    codeResource =
      '<video src=' + srcStr + ' controls style=width:640px;>video</video>';
  } else if (fileType === 'mp3') {
    codeResource =
      '<audio src=' +
      srcStr +
      ' controls>Your browser does not support the audio element</audio>';
  } else {
    codeResource =
      '<p><a href=' +
      srcStr +
