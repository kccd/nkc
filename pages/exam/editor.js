...
var app = new Vue({
  ...
  methods: {
    ...
    save: function () {
      ...
      if (!question.public) {
        if (!question.forum) {
          return screenTopWarning('请选择专业领域');
        }
        question.fid = question.forum.fid;
      }
      if (!question.content) {
        return screenTopWarning('请输入题目内容');
      }
      if (question.type === 'ch4') {
        if (question.answerObj.length !== 4) {
          return screenTopWarning('单项选择题答案个数错误');
        }
        for (var i = 0; i < question.answerObj.length; i++) {
          var a = question.answerObj[i];
          if (!a.text) {
            return screenTopWarning('答案不能为空');
          }
          question.answer[i] = a.text;
        }
      } else {
        if (!question.answerObj[0].text) {
          return screenTopWarning('答案不能为空');
        }
