...
export default {
  ...
  methods: {
    ...
    getShareStickerState() {
      const { share = false } = getFromLocalStorage(
        localStorageKeys.shareStickerWhenUploading,
      );
          ...
          await uploadResourceAsChunks({
            ...
            onProgress: (props) => {
              if (props.type === 'uploading') {
                self.uploadingProgress = props.progress;
              }
            },
          });
      ...
      return sweetQuestion('删除操作无法撤销，确定要执行吗？')
        ...
        .then(() => {
          this.stickers = this.stickers.filter(
            (s) => !this.selectedStickerId.includes(s._id),
          );
    ...
    moveToTop() {
      ...
      nkcAPI('/sticker', 'POST', {
        ...
        .then(() => {
          const newStickers = this.stickers.filter(
            (s) => !this.selectedStickerId.includes(s._id),
          );
