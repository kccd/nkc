  ...
  div(:data-id="commentData._id")
    ...
    .moment-comment-item(
      ...
      .moment-status(v-else-if="commentData && commentData.status === 'disabled'")
        ...
        .hidden-content(v-else)
      .moment-comment-item-content(v-html="commentDataContent" v-if="type === 'comment'")
      //- 图片视频
      .moment-comment-item-files(v-if="type === 'comment'")
        moment-files(:data="commentData.files")
      .moment-comment-item-content.pointer(v-html="commentDataContent" v-else @click="visitUrl(commentData.url, true)")
...
import Lock from '@icon-park/vue/es/icons/Lock';
