...
.single-moment-container(v-if='momentData')
  ...
  #comment-content.single-moment-detail-container(v-if='type === "details"')
      ...
      .single-moment-detail-header
        ...
        .single-moment-detail-tips
          button.btn-xs.btn.btn-default(
            v-if='logged && momentData.subscribed',
            @click.stop='userFollow(false)'
          ) 取关
          button.btn-xs.btn.btn-primary(
            v-if='!logged || (!momentData.subscribed && uid !== momentData.uid)',
            @click.stop='userFollow(true)'
          ) 关注
          .single-moment-tag(
            :class='momentData.visibleType',
            v-if='momentData.visibleType !== "everyone" || (momentData.visibleType === "everyone" && isShowPublicTag)'
          ) {{ visitType[momentData.visibleType] }}
        .single-moment-detail-header-options.fa.fa-ellipsis-h(
    ...
    .single-moment-detail-bottom(
      ...
      .single-moment-moments(v-if='showPanelType')
        moment-comments(
          ...
          :mode='mode'
          @complaint="complaint"
          :comment-control="momentData.commentControl"
        )
  ...
  #comment-content.single-moment-top-container(v-else)
    //moment-option(
    //  ref="momentOption"
    //  @complaint="complaint"
    //)
...
export default {
  ...
  methods: {
    ...
    initData() {
      ...
      if (
        this.logged &&
        window.location.pathname === `/u/${state.uid}/profile/moment`
      ) {
