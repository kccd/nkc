...
const app = new window.Vue({
  ...
  methods: {
    ...
    submit() {
      const { emailCode, mobileCode, password } = this;
      sweetQuestion(
        '确定即会注销，注销后短期内不能使用原有用户名和手机号重新注册，你将不能再对该账号、积分、发表的内容进行处置，你想好了吗？',
      )
        .then(() => {
          return nkcAPI(`/u/${window.NKC.configs.uid}/destroy`, 'POST', {
            type: 'destroy',
            form: {
              emailCode,
              mobileCode,
