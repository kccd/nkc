...
authRouter
  .get('/', OnlyOperation(Operations.visitUserAuth), async (ctx, next) => {
    ...
    const verifiedUploadVideo = await db.VerifiedUploadModel.findOne(
      { _id: userPersonal.authenticate.video.attachments[0] },
      { state: 1, _id: 1, errorInfo: 1 },
    ).sort({ toc: -1 });
    const verifiedUploadCardA = await db.VerifiedUploadModel.findOne(
      { _id: userPersonal.authenticate.card.attachments[0] },
      { state: 1, _id: 1, errorInfo: 1 },
    ).sort({ toc: -1 });
    const verifiedUploadCardB = await db.VerifiedUploadModel.findOne(
      { _id: userPersonal.authenticate.card.attachments[1] },
      { state: 1, _id: 1, errorInfo: 1 },
    ).sort({ toc: -1 });
    data.verifiedVideoState = verifiedUploadVideo
      ? verifiedUploadVideo.state
      : 'unUpload';
    data.verifiedAState = verifiedUploadCardA
      ? verifiedUploadCardA.state
      : 'unUpload';
    data.verifiedBState = verifiedUploadCardB
      ? verifiedUploadCardB.state
      : 'unUpload';
    data.videoErrorInfo = verifiedUploadVideo
      ? verifiedUploadCardB.errorInfo
      : '';
    data.cardAErrorInfo = verifiedUploadCardA
      ? verifiedUploadCardA.errorInfo
      : '';
    data.cardBErrorInfo = verifiedUploadCardB
      ? verifiedUploadCardB.errorInfo
      : '';
  ...
  .del('/', OnlyOperation(Operations.cancelSubmitVerify), async (ctx, next) => {
  ...
  .post(
    '/verify2',
    OnlyOperation(Operations.modifyUserVerifyStatus),
    async (ctx, next) => {
  ...
  .get(
    '/a/:vid',
    OnlyOperation(Operations.auditorVisitVerifiedUpload),
    async (ctx, next) => {
      const { params, data, db } = ctx;
      const { vid } = params;
