...
sysInfoRouter
  .get('/', OnlyOperation(Operations.visitSystemInfo), async (ctx, next) => {
    const { data, db } = ctx;
    ctx.template = 'experimental/systemInfo/systemInfo.pug';
    data.systemMessages = await db.MessageModel.find({ ty: 'STE' }).sort({
      tc: -1,
    });
    data.grades = await UsersGradeModel.find(
      {},
      { _id: true, displayName: true },
    ).sort({ score: 1 });
    data.roles = await RoleModel.find(
      {},
      { _id: true, displayName: true },
    ).sort({ score: 1 });
    await next();
  })
  .post('/', OnlyOperation(Operations.sendSystemInfo), async (ctx, next) => {
  ...
  .put('/', OnlyOperation(Operations.modifySystemInfo), async (ctx, next) => {
    const { db, body, data } = ctx;
    const { form } = body;
    if (!form.content) return ctx.throw(400, '通知内容不能为空');
    if (!form.id) return ctx.throw(400, '未接收到消息ID');
    data.content = form.content;
    const result = await db.MessageModel.updateOne(
      { _id: form.id, ty: 'STE' },
      {
        $set: {
  ...
  .del('/', OnlyOperation(Operations.deleteSystemInfo), async (ctx, next) => {
    const { query } = ctx;
    const { id } = query;
  ...
  .get(
    '/fuzzy_search_user',
    OnlyOperation(Operations.fuzzySearchUser),
    async (ctx, next) => {
      ...
      if (post) {
        const userByPid = await UserModel.findOne(
          { uid: post.uid },
          { username: true },
        );
        if (userByPid) {
      ...
      if (thread) {
        const mainPost = await PostModel.findOne(
          { pid: thread.oc },
          { t: true, uid: true },
        );
        if (mainPost) {
          const title =
            mainPost.t.length < 10
              ? mainPost.t
              : mainPost.t.substring(0, 5) + '...' + mainPost.t.substr(-2);
          const userByThread = await UserModel.findOne(
            { uid: mainPost.uid },
            { username: true },
          );
          if (userByThread) {
      ...
      if (!Number.isNaN(castToNumber)) {
        ...
        if (column) {
          const userByColumn = await UserModel.findOne(
            { uid: column.uid },
            { username: true },
          );
          if (userByColumn) {
