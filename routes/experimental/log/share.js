...
router.get('/', OnlyOperation(Operations.visitShareLogs), async (ctx, next) => {
  ...
  data.paging = paging;
  const shareLogs = await db.ShareLogsModel.find(q)
    .sort({ toc: -1 })
    .skip(paging.start)
    .limit(paging.perpage);
  data.shareLogs = await Promise.all(
    shareLogs.map(async (log) => {
      log = log.toObject();
      if (log.uid) log.user = await db.UserModel.findOne({ uid: log.uid });
      if (log.shareUid)
