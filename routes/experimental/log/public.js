...
router
  .get('/', OnlyOperation(Operations.visitPublicLogs), async (ctx, next) => {
    ...
    if (c.uid) {
      const asUsername = await db.UserModel.findOne(
        { usernameLowerCase: c.uid.toLowerCase() },
        { uid: 1 },
      );
    ...
    if (!noopReturn) {
      if (t !== 'visitor' && logType === 'user') {
        ...
        const logs = await db.LogModel.find(searchMap)
          .sort({ reqTime: -1 })
          .skip(paging.start)
          .limit(paging.perpage);
        data.result = await Promise.all(
          logs.map(async (behavior) => {
        ...
        paging = nkcModules.apiFunction.paging(page, count, 60);
        data.result = await db.VisitorLogModel.find(searchMap)
          .sort({ reqTime: -1 })
          .skip(paging.start)
          .limit(paging.perpage);
      }
  ...
  .del('/', OnlyOperation(Operations.deletePublicLogs), async (ctx, next) => {
    const { db, query } = ctx;
    let { del } = query;
    del = del
      ? JSON.parse(decodeURIComponent(Buffer.from(del, 'base64').toString()))
      : {};
    const delMap = {
      $and: [{ reqTime: {} }],
    };
