...
router.get(
  '/',
  OnlyOperation(Operations.viewEmailcodeRecord),
  async (ctx, next) => {
    const { db, nkcModules, query, data } = ctx;
    let { page = 0, c } = query;
    c = c
      ? JSON.parse(decodeURIComponent(Buffer.from(c, 'base64').toString()))
      : {};
    let searchMap = {};
    if (c.sTime) {
    ...
    let result = await db.EmailCodeModel.find(searchMap)
      .sort({ toc: -1 })
      .skip(paging.start)
    ...
    data.c = encodeURIComponent(
      new Buffer(JSON.stringify(c)).toString('base64'),
    );
