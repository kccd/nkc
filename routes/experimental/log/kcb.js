...
kcbRouter
  .get(
    ...
    async (ctx, next) => {
      ...
      const kcbsRecords = await db.KcbsRecordModel.find(q)
        .sort({ toc: -1 })
        .skip(paging.start)
        .limit(paging.perpage);
      data.kcbsRecords = await db.KcbsRecordModel.extendKcbsRecords(
        kcbsRecords,
      );
  ...
  .get(
    '/diff',
    OnlyOperation(Operations.visitExperimentalDiffKcb),
    async (ctx, next) => {
      const { data, db } = ctx;
  ...
  .post(
    '/diff',
    OnlyOperation(Operations.resetExperimentalDiffKcb),
    async (ctx, next) => {
      const { body, db } = ctx;
      const { uid } = body;
