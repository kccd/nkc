...
router.get(
  '/',
  OnlyOperation(Operations.visitRecycleMarkThreads),
  async (ctx, next) => {
    ...
    const delLogs = await db.DelPostLogModel.find(q)
      .sort({ toc: -1 })
      .skip(paging.start)
    ...
    for (let log of delLogs) {
      log = log.toObject();
      const { userId, delUserId, postType, postId, threadId, reason, delType } =
        log;
      ...
      thread = (
        await db.ThreadModel.extendThreads([thread], {
          forum: false,
          lastPost: false,
          lastPostUser: false,
          category: false,
