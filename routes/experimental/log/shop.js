...
router.get(
  '/',
  OnlyOperation(Operations.visitExperimentalShop),
  async (ctx, next) => {
    ...
    try {
      if (c) {
        ...
        const { orderId, userType, userContent, tUserType, tUserContent } =
          JSON.parse(obj);
        if (orderId) {
          q.orderId = orderId;
        }
        if (userContent) {
          ...
          if (userType === 'username') {
            const user = await db.UserModel.findOne({
              usernameLowerCase: userContent.toLowerCase(),
            });
            if (!user) {
              buyUid = 'null';
            } else {
        ...
        if (tUserContent) {
          ...
          if (tUserType === 'username') {
            const user = await db.UserModel.findOne({
              usernameLowerCase: tUserContent.toLowerCase(),
            });
            if (!user) {
              sellUid = 'null';
            } else {
    ...
    const orders = await db.ShopOrdersModel.find(q)
      .sort({ orderToc: -1 })
      .skip(paging.start)
