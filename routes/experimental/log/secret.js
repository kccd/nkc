...
router
  .get('/', OnlyOperation(Operations.visitSecretLogs), async (ctx, next) => {
    ...
    const secrets = await db.SecretBehaviorModel.find(q)
      .sort({ toc: -1 })
      .skip(paging.start)
    ...
    for (let secret of secrets) {
      ...
      if (['bindEmail', 'changeEmail', 'unbindEmail'].includes(type)) {
        ...
        ['bindMobile', 'modifyMobile', 'unbindMobile'].includes(type)
      ) {
        if (secret.oldMobile) {
  ...
  .post('/', OnlyOperation(Operations.visitSecretLogs), async (ctx, next) => {
    ...
    if (type === 'restore') {
      ...
      const sameUsernameCount = await db.UserModel.countDocuments({
        usernameLowerCase: oldUsernameLowerCase,
      });
      const warning = [];
      ...
      const sameEmailCount = await db.UsersPersonalModel.countDocuments({
        email: oldEmail,
      });
      if (sameEmailCount > 0) {
      ...
      const sameMobileCount = await db.UsersPersonalModel.countDocuments({
        mobile: oldMobile,
        nationCode: oldNationCode,
      });
