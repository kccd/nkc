...
router
  .use(
    '/',
    OnlyOperation(Operations.experimentalForumsSettings),
    async (ctx, next) => {
      const { db, data } = ctx;
  ...
  .get(
    ...
    async (ctx, next) => {
      ...
      data.forumCategories = await db.ForumCategoryModel.find().sort({
        order: 1,
      });
  ...
  .put(
    ...
    async (ctx, next) => {
      ...
      if (!recycle) {
        ctx.throw(400, '回收站专业ID不能为空');
      }
      if (!archive) {
        ctx.throw(400, '归档专业ID不能为空');
      }
      const forum = await db.ForumModel.findOne({ fid: recycle });
      const archiveForum = await db.ForumModel.findOne(
        { fid: archive },
        { fid: true },
      );
      if (!forum) {
        ctx.throw(400, `回收站专业不存在 fid: ${recycle}`);
      }
      if (!archiveForum) {
        ctx.throw(400, `归档专业不存在 fid: ${archive}`);
      }
      ...
      for (let i = 0; i < categories.length; i++) {
        ...
        if (_id !== undefined) {
          // 已存在
          await db.ForumCategoryModel.updateOne(
            { _id },
            {
              $set: {
                name,
      ...
      for (let i = 0; i < forumCategories.length; i++) {
        ...
        const { mutuallyExclusiveWithSelf, mutuallyExclusiveWithOthers } = fc;
        ...
        for (const f of forumCategories) {
          if (f === fc || !f.mutuallyExclusiveWithOthers) {
            continue;
          }
