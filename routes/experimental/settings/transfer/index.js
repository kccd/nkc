...
router
  .use(
    '/',
    OnlyOperation(Operations.experimentalTransferKCB),
    async (ctx, next) => {
      const { user } = ctx.data;
  ...
  .get(
    ...
    async (ctx, next) => {
      ...
      data.shopScore = await db.SettingModel.getScoreByOperationType(
        'shopScore',
      );
  ...
  .post(
    ...
    async (ctx, next) => {
      ...
      if (!enabledScoresType.includes(scoreType))
        ctx.throw(400, '已选积分暂未开启，请刷新');
      ...
      if (fromUser) {
        const fromUserScore = await db.UserModel.getUserScore(
          fromUser.uid,
          scoreType,
        );
  ...
  .put(
    ...
    async (ctx, next) => {
      ...
      const { kcbOnce, countOneDay, countToUserOneDay } = transferSettings;
      checkNumber(kcbOnce, {
        name: '单次转账KCB上限',
        min: 0,
        fractionDigits: 2,
