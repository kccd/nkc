...
shopRouter
  .use('/', OnlyOperation(Operations.visitShopSettings), async (ctx, next) => {
    ...
    data.applysDealing = await db.ShopApplyStoreModel.find({
      applyStatus: 'dealing',
    });
    data.shopSettings = await db.SettingModel.getSettings('shop');
    await next();
  })
  .get('/', OnlyOperation(Operations.visitShopSettings), async (ctx, next) => {
    ctx.template = 'experimental/shop/base/base.pug';
    await next();
  })
  .put('/', OnlyOperation(Operations.visitShopSettings), async (ctx, next) => {
  ...
  .use(
    '/refunds/settings',
    refundSettingsRouter.routes(),
    refundSettingsRouter.allowedMethods(),
  )
  .use(
    '/homeSetting',
    homeSettingRouter.routes(),
    homeSettingRouter.allowedMethods(),
  );
