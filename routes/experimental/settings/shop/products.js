...
productsRouter
  .get('/', OnlyOperation(Operations.visitiShopProducts), async (ctx, next) => {
    ...
    const products = await db.ShopGoodsModel.find(queryMap)
      .sort({ toc: -1 })
      .skip(paging.start)
  ...
  .put(
    '/bansale',
    OnlyOperation(Operations.shopAdminBanProductSale),
    async (ctx, next) => {
  ...
  .put(
    '/clearban',
    OnlyOperation(Operations.shopAdminClearBanSale),
    async (ctx, next) => {
