...
homeSettingRouter
  .use('/', OnlyOperation(Operations.visitShopSettings), async (ctx, next) => {
    const { db, data } = ctx;
  ...
  .get(
    '/carousel',
    OnlyOperation(Operations.visitHomeSettingCarousel),
    async (ctx, next) => {
      const { data, db } = ctx;
  ...
  .post(
    '/carousel',
    OnlyOperation(Operations.changeHomeSettingCarousel),
    async (ctx, next) => {
  ...
  .put(
    '/carousel',
    OnlyOperation(Operations.deleteHomeSettingCarousel),
    async (ctx, next) => {
      const { data, body } = ctx;
      const { index } = body;
      let homeSetting = data.homeSetting;
  ...
  .get(
    '/featured',
    OnlyOperation(Operations.visitHomeSettingFeatured),
    async (ctx, next) => {
      const { data, db } = ctx;
      // 取出精选商品并拓展详细信息(取出的结果按id大小进行了重新排序)
      let featuredItems = await db.ShopGoodsModel.find({
        productId: { $in: data.homeSetting.featureds },
      });
  ...
  .post(
    '/featured',
    OnlyOperation(Operations.changeHomeSettingFeatured),
    async (ctx, next) => {
  ...
  .get(
    '/recommendation',
    OnlyOperation(Operations.visitHomeSettingRecommendation),
    async (ctx, next) => {
      const { data, query, db, params, body } = ctx;
      const recommendationItems = await db.ShopStoresModel.find({
        storeId: { $in: data.homeSetting.recommendations },
      });
  ...
  .post(
    '/recommendation',
    OnlyOperation(Operations.changeHomeSettingRecommendation),
    async (ctx, next) => {
      const { data, query, db, params, body } = ctx;
      const { type } = body;
      // 添加推荐店铺
      if (type == 'add') {
        ...
        const recommendationIndex =
          homeSetting.recommendations.indexOf(storeId);
        if (recommendationIndex < 0) {
          homeSetting.recommendations.push(storeId);
          await homeSetting.updateOne({
            recommendations: homeSetting.recommendations,
          });
          data.homeSetting = homeSetting;
        } else {
  ...
  .get(
    '/popular',
    OnlyOperation(Operations.visitHomeSettingPopular),
    async (ctx, next) => {
      const { data, query, db, params, body } = ctx;
      let popularItems = await db.ShopGoodsModel.find({
        productId: { $in: data.homeSetting.populars },
      });
  ...
  .post(
    '/popular',
    OnlyOperation(Operations.changeHomeSettingPopular),
    async (ctx, next) => {
      const { data, query, db, params, body } = ctx;
      const { type } = body;
      // 添加热门商品
