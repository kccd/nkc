...
router
  .get(
    '/',
    OnlyOperation(Operations.experimentalVisitSettings),
    async (ctx, next) => {
      const { db, data } = ctx;
      ...
      data.accessControl =
        await db.AccessControlModel.getAllAccessControlDetail();
  ...
  .put(
    ...
    async (ctx, next) => {
      ...
      const checkAndModifyAC = (source, acForm) => {
        ...
        whitelist.rolesId = whitelist.rolesId.filter((roleId) =>
          rolesId.includes(roleId),
        );
        whitelist.gradesId = whitelist.gradesId.filter((gradeId) =>
          gradesId.includes(gradeId),
        );
      };
      ...
      for (const ac of accessControl) {
        const { source, app, web } = ac;
        await db.AccessControlModel.updateOne(
          { source },
          {
            $set: {
