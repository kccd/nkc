...
router
  .get(
    '/',
    OnlyOperation(Operations.experimentalUploadSettings),
    async (ctx, next) => {
      const { db, data } = ctx;
      const uploadSettings = await db.SettingModel.getSettings('upload');
      data.certList = await db.RoleModel.getCertList(['visitor']);
      uploadSettings.countLimit.others = await db.RoleModel.filterSettings(
        uploadSettings.countLimit.others,
        ['visitor'],
      );
      uploadSettings.extensionLimit.others = await db.RoleModel.filterSettings(
        uploadSettings.extensionLimit.others,
        ['visitor'],
      );
      data.watermarkScore = await db.SettingModel.getScoreByOperationType(
        'watermarkScore',
      );
  ...
  .put(
    ...
    async (ctx, next) => {
      ...
      countLimit.others = await db.RoleModel.filterSettings(countLimit.others, [
        'visitor',
      ]);
      extensionLimit.others = await db.RoleModel.filterSettings(
        extensionLimit.others,
        ['visitor'],
      );
      // 水印尺寸限制
      ...
      if (files.normalPictureWatermark) {
        if (files.normalPictureWatermark.size > watermarkSize)
          ctx.throw(400, '图片默认水印大小不能超过1M!');
        await fsPromise.copyFile(
          files.normalPictureWatermark.path,
          statics.normalPictureWatermark,
        );
      }
      if (files.smallPictureWatermark) {
        if (files.smallPictureWatermark.size > watermarkSize)
          ctx.throw(400, '图片小水印大小不能超过1M!');
        await fsPromise.copyFile(
          files.smallPictureWatermark.path,
          statics.smallPictureWatermark,
        );
      }
      if (files.normalVideoWatermark) {
        if (files.normalVideoWatermark.size > watermarkSize)
          ctx.throw(400, '视频默认水印大小不能超过1M!');
        await fsPromise.copyFile(
          files.normalVideoWatermark.path,
          statics.normalVideoWatermark,
        );
      }
      if (files.smallVideoWatermark) {
        if (files.smallVideoWatermark.size > watermarkSize)
          ctx.throw(400, '视频小水印大小不能超过1M!');
        await fsPromise.copyFile(
          files.smallVideoWatermark.path,
          statics.smallVideoWatermark,
        );
      }
