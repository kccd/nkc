...
smsRouter
  .get('/', OnlyOperation(Operations.visitSmsSettings), async (ctx, next) => {
    const { data, db } = ctx;
  ...
  .put('/', OnlyOperation(Operations.modifySmsSettings), async (ctx, next) => {
    const { db, body, nkcModules } = ctx;
    const { checkString } = nkcModules.checkData;
    const { smsSettings } = body;
    const reg = /^\**?$/;
    const smsSettingsDB = await db.SettingModel.findOnly({ _id: 'sms' });
    const { templates, restrictedNumber, appId, appKey, smsSign, platform } =
      smsSettings;
    if (!['aliCloud', 'tencentCloud', 'alidayu'].includes(platform))
      ctx.throw(400, `短信平台错误 platform: ${platform}`);
    checkString(appId, {
      name: 'App ID',
      minLength: 1,
    });
    ...
    for (const template of templates) {
      if (template.id === '')
        ctx.throw(400, template.name + '的模板ID不能为空');
      for (const o of template.oid) {
        if (!o.nationCode || !o.id)
          ctx.throw(400, template.name + '的模板ID不能为空');
      }
      if (template.validityPeriod === '')
        ctx.throw(400, template.name + '的有效时间不能为空');
      if (template.validityPeriod <= 0)
        ctx.throw(400, template.name + '的有效时间必须大于0');
      if (template.sameIpOneDay === '')
        ctx.throw(400, template.name + '的IP次数限制不能为空');
      if (template.sameIpOneDay <= 0)
        ctx.throw(400, template.name + '的IP次数限制必须大于0');
      if (template.sameMobileOneDay === '')
        ctx.throw(400, template.name + '的手机号码次数限制不能为空');
      if (template.sameMobileOneDay <= 0)
        ctx.throw(400, template.name + '的手机号码次数限制必须大于0');
    }
  ...
  .post(
    '/test',
    OnlyOperation(Operations.testSendMessage),
    async (ctx, next) => {
