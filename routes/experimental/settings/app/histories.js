...
historiesRouter
  .get('/', async (ctx, next) => {
    ...
    data.type = 'histories';
    data.os = os;
    const count = await db.AppVersionModel.countDocuments({ appPlatForm: os });
  ...
  .put('/', async (ctx, next) => {
    ...
    if (operating === 'setStable') {
      await db.AppVersionModel.updateMany({ stable: true }, {
        $set: {
          stable: false
        }
      });
    ...
    if (operating === 'updateVersion') {
      const repeatVersionCount = await db.AppVersionModel.countDocuments({ appVersion: newVersion.appVersion, hash: { $ne: newVersion.hash } });
