...
emailRouter
  .get(
    '/',
    OnlyOperation(Operations.visitExperimentalEmailSettings),
    async (ctx, next) => {
      const { data, db } = ctx;
  ...
  .put(
    '/',
    OnlyOperation(Operations.modifyEmailSettings),
    async (ctx, next) => {
      const { db, body } = ctx;
  ...
  .post('/test', OnlyOperation(Operations.testSendEmail), async (ctx, next) => {
