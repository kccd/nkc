...
router
	.get('/', async (ctx, next) => {
		const {data, db} = ctx;
		data.downloadSettings = await db.SettingModel.findOnly({type: 'download'});
		ctx.template = 'experimental/settings/download.pug';
		await next();
	})
	.patch('/', async (ctx, next) => {
		const {db, body} = ctx;
		const downloadSettings = await db.SettingModel.findOnly({type: 'download'});
		if(body.operation === 'saveDownloadFile') {
			...
			await downloadSettings.update({numberOfDays, numberOfKcb});
		}
		await next();
	});
