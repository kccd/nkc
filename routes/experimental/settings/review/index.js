...
router
  .get(
    '/',
    OnlyOperation(Operations.experimentalReviewSettings),
    async (ctx, next) => {
      const { data, db } = ctx;
  ...
  .put(
    ...
    async (ctx, next) => {
      ...
      if (!['thread', 'post'].includes(tab))
        ctx.throw(
          400,
          '参数错误，服务器无法确定是发表文章还是发表回复的审核设置',
        );
      ...
      if (type === 'addUser') {
        ...
        if (listType === 'blacklist') {
          if (reviewSettings[tab].special.whitelistUid.includes(uid))
            ctx.throw(400, '用户已存在于白名单中，无法添加到黑名单');
          const obj = {};
          ...
          if (reviewSettings[tab].special.blacklistUid.includes(uid))
            ctx.throw(400, '用户已存在于黑名单中，无法添加到白名单');
          const obj = {};
        ...
        if (notPassedA.type !== 'all' && notPassedA.type !== 'some')
          ctx.throw(400, 'type参数不正确，可能是前端兼容性问题。');
        if (notPassedA.count <= 0) ctx.throw(400, '通过的次数不能小于1');
        foreign.status = !!foreign.status;
        if (foreign.type !== 'all' && foreign.type !== 'some')
          ctx.throw(400, 'type参数不正确，可能是前端兼容性问题。');
        ...
        for (const grade of grades) {
          ...
          if (!g)
            ctx.throw(
              400,
              `在数据库中未找到ID为${grade.gradeId}的用户等级，请注意保存已填写的设置，然后刷新页面重试。`,
            );
          grade.status = !!grade.status;
          if (!['all', 'some'].includes(grade.type))
            ctx.throw(400, 'type参数不正确，可能是前端兼容性问题。');
