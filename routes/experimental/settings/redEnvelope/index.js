...
redEnvelopeRouter
  .get('/', OnlyOperation(Operations.visitERedEnvelope), async (ctx, next) => {
    const { data, db, nkcModules } = ctx;
    data.redEnvelopeSettings = await db.SettingModel.getSettings('redEnvelope');
    data.postRewardScore = await db.SettingModel.getScoreByOperationType(
      'postRewardScore',
    );
    data.digestRewardScore = await db.SettingModel.getScoreByOperationType(
      'digestRewardScore',
    );
    data.shareRewardScore = await db.SettingModel.getScoreByOperationType(
      'shareRewardScore',
    );
    const shareSettings = await db.SettingModel.getSettings('share');
    const shares = [];
    for (const type in shareSettings) {
      const { kcb, maxKcb, rewardCount, rewardStatus } = shareSettings[type];
      shares.push({
  ...
  .put(
    ...
    async (ctx, next) => {
      ...
      if (draftFee.maxCount < draftFee.minCount)
        ctx.throw(400, '红包最大数设置错误');
      // 分享奖励
      const shareSettings = await db.SettingModel.getSettings('share');
      for (const s of shares) {
        const { type, kcb, maxKcb, rewardStatus, rewardCount } = s;
        checkNumber(kcb, {
          name: '分享奖励 - 单次点击奖励',
          min: 0,
        });
