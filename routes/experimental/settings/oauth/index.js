...
router
  .use('/', OnlyOperation(Operations.manageOauthApp), async (ctx, next) => {
    const { db } = ctx;
    ...
    for (let appStatusKey in appStatus) {
      oauthStatus[appStatusKey] = translate(
        languageNames.zh_cn,
        'oauth',
    ...
    for (let appOperationKey in appOperations) {
      oauthOperations[appOperationKey] = translate(
        languageNames.zh_cn,
        'oauth',
        appOperations[appOperationKey],
      );
    }
