...
router
  .get(
    '/',
    OnlyOperation(Operations.experimentalComplaintSettings),
    async (ctx, next) => {
      const { db, data } = ctx;
  ...
  .put(
    ...
    async (ctx, next) => {
      ...
      for (let i = 0; i < complaintTypesId.length; i++) {
        await db.ComplaintTypeModel.updateOne(
          { _id: complaintTypesId[i] },
          {
            $set: {
              order: i + 1,
            },
  ...
  .put(
    ...
    async (ctx, next) => {
      ...
      if (operation === 'modifyDisabled') {
        ...
        const sameTypeCount = await db.ComplaintTypeModel.countDocuments({
          _id: { $ne: _id },
          type,
        });
        if (sameTypeCount > 0) {
