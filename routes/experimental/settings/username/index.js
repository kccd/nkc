...
router
  .get(
    '/',
    OnlyOperation(Operations.experimentalUsernameSettings),
    async (ctx, next) => {
      const { db, data } = ctx;
      data.usernameSettings = await db.SettingModel.getSettings('username');
      data.scoreObject = await db.SettingModel.getScoreByOperationType(
        'usernameScore',
      );
  ...
  .put(
    ...
    async (ctx, next) => {
      ...
      if (freeCount < 0) {
        ctx.throw(400, '免费修改次数不能小于0');
      }
      if (maxKcb < 0) {
        ctx.throw(400, '花费积分最大值不能小于0');
      }
      if (onceKcb < 0) {
        ctx.throw(400, '花费积分增量不能小于0');
      }
