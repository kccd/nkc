...
router
  .get(
    '/',
    OnlyOperation(Operations.experimentalNoteSettings),
    async (ctx, next) => {
      const { data, db } = ctx;
  ...
  .put(
    ...
    async (ctx, next) => {
      const {
        db,
        body: { enabled, keyWordGroup, keyWordGroupChecked },
      } = ctx;
      const arr = keyWordGroup.map((e) => e.id); //敏感词库
      const unKnownArr = keyWordGroupChecked.filter(
        (item) => arr.indexOf(item) === -1,
      ); //返回的是前端提交的keyWordGroupChecked中keyWordGroup没有的词
      ...
      const data = await db.SettingModel.updateOne(
        { _id: 'note' },
        {
          $set: {
            'c.enabled': !!enabled,
