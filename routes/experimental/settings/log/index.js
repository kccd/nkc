...
logSettingRouter
    .use('/', async (ctx, next) => {
        ...
        // 如果操作在日志中已经被使用，则添加标记
        let logSettings = await db.SettingModel.findOne({"type":"log"});
    ...
    .post('/', async(ctx, next) => {
        ...
        if(logParams){
            let logSetting = await db.SettingModel.find({"type":"log"})
            await logSetting[0].update({"operationsId":logParams})
