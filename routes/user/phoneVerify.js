...
router
  .get('/', OnlyUser(), async (ctx, next) => {
    ...
    if ((await userPersonal.getAuthLevel()) < 1) {
      ctx.throw(403, '你还未绑定手机号');
    }
    const { nationCode, mobile } = userPersonal;
    data.phone = `+${nationCode || 86} ${mobile.substring(
      0,
      3,
    )}****${mobile.substring(7)}`;
  ...
  .post('/', OnlyUser(), async (ctx, next) => {
    ...
    const { user } = data;
    const { code } = body;
    const { nationCode, number } =
      await db.UsersPersonalModel.getUserPhoneNumber(user.uid);
    const smsObj = {
      nationCode,
      mobile: number,
      code,
