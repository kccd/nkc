...
userRouter
  ...
  .use('/:uid', OnlyUnbannedUser(), async (ctx, next) => {
    ...
    if (!data.targetUser) {
      ctx.throw(404, `不存在ID为${params.uid}的用户`);
    }
    //拓展用户权限
  ...
  .get('/:uid', OnlyUnbannedUser(), async (ctx, next) => {
    ...
    if (user) {
      data.inBlacklist = !!(await db.BlacklistModel.findOne({
        uid: user.uid,
        tUid: targetUser.uid,
      }));
    }
    ...
    if (from && from === 'panel' && ctx.request.get('FROM') === 'nkcAPI') {
      if (data.user) {
        ...
        if (friend) {
          ...
          if (!data.friend.phone || !data.friend.phone.length) {
            data.friend.phone = [''];
          }
        }
      }
      ...
      data.friendCategories = await db.FriendsCategoryModel.find({
        uid: user.uid,
      }).sort({ toc: -1 });
  ...
  .use(
    '/:uid/userPanel',
    userPanelRouter.routes(),
    userPanelRouter.allowedMethods(),
  );
