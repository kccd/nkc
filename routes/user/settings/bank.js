  ...
  if (type === 'deleteAccount') {
    for (var i = 0; i < bankAccounts.length; i++) {
      ...
      if (a.account === account.account) {
        if (a.default && bankAccounts.length > 1) {
          ctx.throw(400, '无法删除默认支付宝账号');
        }
    ...
    for (const a of accounts) {
      ...
      if (!bankName) {
        ctx.throw(400, '银行名称不能为空');
      }
      if (contentLength(bankName) > 50) {
        ctx.throw(400, '银行名称不能超过50字节');
      }
      if (!account) {
        ctx.throw(400, '银行卡号不能为空');
      }
      if (contentLength(account) > 100) {
        ctx.throw(400, '银行卡号不能超过50字节');
      }
      if (!name) {
        ctx.throw(400, '开户姓名不能为空');
      }
      if (contentLength(name) > 50) {
        ctx.throw(400, '开户姓名不能超过50字节');
      }
      if (!mobile) {
        ctx.throw(400, '开户手机号不能为空');
      }
      if (contentLength(mobile) > 50) {
        ctx.throw(400, '开户手机号不能为空');
      }
      a.time = time;
    }
    if (!hasDefault && accounts.length) {
