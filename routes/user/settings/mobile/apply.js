...
router
  .use('/', OnlyUnbannedUser(), async (ctx, next) => {
    ...
    data.pendingApplication =
      !!(await db.SecurityApplicationModel.getPendingApplication(state.uid));
    if (authLevel < 1) {
      ctx.throw(403, `你暂未绑定手机号`);
    }
    await next();
  })
  ...
  .post('/', OnlyUnbannedUser(), async (ctx, next) => {
    ...
    const { user } = data;
    await db.VerificationRecordModel.verifyRecord(
      user.uid,
      verificationRecordId,
    );
    if (addresses.length < 1) {
      ctx.throw(400, '常用登录地点不能为空');
    }
    if (addresses.length > 3) {
      ctx.throw(400, '常用登录地点不能超过3个');
    }
    ...
    if (description && description.length > 1000) {
      ctx.throw(400, '申诉说明不能超过1000字');
    }
    if (!password) {
      ctx.throw(400, '登录密码不能为空');
    }
    ...
    // 标记验证码为已使用
    await smsCode.mark();
    const oldPhoneNumber = await db.UsersPersonalModel.getUserPhoneNumber(
      user.uid,
    );
