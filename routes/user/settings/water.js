...
waterRouter
  ...
  .put('/', OnlyUnbannedUser(), async (ctx, next) => {
    ...
    if (type === 'pay') {
      // 检测是否已经购买了水印服务
      const waterAlreadyPay = await db.UsersGeneralModel.findOne({
        uid: user.uid,
      });
      ...
      if (needScore && myScore < needScore) {
        ctx.throw(
          400,
          `您的${scoreObject.name}不足${needScore / 100}${scoreObject.unit}`,
        );
      }
      // 够了，生成一条转给银行的转账记录
      await db
    ...
    if (type === 'save') {
      ...
      if (userWaterSetting) {
        await db.UsersGeneralModel.updateOne(
          ...
          {
            $set: {
              'waterSetting.waterAdd': waterAdd,
              'waterSetting.picture.waterGravity': pictureWaterGravity,
              'waterSetting.picture.waterStyle': pictureWaterStyle,
              'waterSetting.video.waterGravity': videoWaterGravity,
              'waterSetting.video.waterStyle': videoWaterStyle,
            },
          },
        );
      } else {
