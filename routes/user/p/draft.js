...
module.exports = async (ctx, next) => {
  ...
  if(type === 'community') {
    ...
    // 文章有两种类型
      ...
      // post 可能是修改文章
    // newThread 等于 forum
    const threadData = await db.DraftModel.getLatestNewThread(targetUser.uid);
    drafts = threadData ? [threadData] : [];
  } else if (type === 'newPost') {
    // 获取最近的新回复
    const { desTypeId } = query;
    if(!desTypeId) ctx.throw(400, 'desTypeId不存在');
    const postData = await db.DraftModel.getLatestNewPost(desTypeId, targetUser.uid);
    drafts = postData ? [postData] : [];
    
  } else if (type === 'modifyThread') {
    // 获取最近的修改文章
    const { desTypeId } = query;
    const threadData = await db.DraftModel.getLatestModifyThread(desTypeId, targetUser.uid, nkcModules);
    drafts = threadData ? [threadData] : [];

  } else if (type === 'modifyPost') {
    const { desTypeId } = query;
    if(!desTypeId) ctx.throw(400, 'desTypeId不存在');   
    const postData = await db.DraftModel.getLatestModifyPost(desTypeId, targetUser.uid, nkcModules);
    drafts = postData ? [postData] : [];
