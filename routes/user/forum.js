...
router
  ...
  .get('/apply', OnlyUser(), async (ctx, next) => {
    ...
    data.appliedForums = await db.PreparationForumModel.find({
      uid: state.uid,
    });
  ...
  .post('/apply', OnlyUnbannedUser(), async (ctx, next) => {
    ...
    for (const tUid of founders) {
      ...
      if (u === 0) {
        ctx.throw(400, `uid错误 uid:${tUid}`);
      }
    }
    if (founders.includes(uid)) {
      ctx.throw(400, `无需邀请自己`);
    }
    if (founders.length < 3) {
      ctx.throw(400, `请至少邀请3人作为新专业的共同创始人`);
    }
    ...
    const sameForum = await db.ForumModel.countDocuments({
      displayName: newForumName,
    });
    if (sameForum) {
      ctx.throw(400, '专业名已存在，请更换');
    }
    checkString(reason, {
      name: '开办理由',
      minLength: 400,
    ...
    const pfid = await db.PreparationForumModel.createPForum(
      uid,
      info,
      founders,
    );
  ...
  .use('/invitation', OnlyUser(), async (ctx, next) => {
    ...
    if (founder.accept === 'timeout') {
      ctx.throw(403, `邀请已过期`);
    } else if (['resolved', 'rejected'].includes(founder.accept)) {
      ctx.throw(403, `邀请已处理`);
    }
  ...
  .get('/invitation', OnlyUser(), async (ctx, next) => {
    ...
    data.targetUser = await db.UserModel.findOnly(
      { uid: pForum.uid },
      {
        uid: 1,
        username: 1,
  ...
  .post('/invitation', OnlyUnbannedUser(), async (ctx, next) => {
    const { db, body, state, data } = ctx;
    const { res, pfid } = body;
    if (!['resolved', 'rejected'].includes(res)) {
      ctx.throw(400, `res参数错误 res: ${res}`);
    }
    ...
    for (let founder of founders) {
      if (founder.uid === state.uid) {
        founder.accept = res;
        await db.PreparationForumModel.updateOne(
          { pfid },
          {
            $set: { founders },
          },
