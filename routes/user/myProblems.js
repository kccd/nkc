...
myProblems
  .get('/', async (ctx, next) => {
    ...
    // }
    const problems = await db.ProblemModel.find({uid: user.uid}).sort({toc: -1}).skip(paging.start).limit(paging.perpage);
  ...
  // 问题详情 路由
    ...
    const {user} = data;
     ...
     if (user.uid !== data.problem.uid) ctx.throw(403, '权限不足') 
