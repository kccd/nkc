...
router.get('/', async (ctx, next) => {
  ...
  const subForumsId = targetUserSubForums
    .map((f) => f.sid)
    .filter((fid) => accessibleFid.includes(fid));
  ...
  data.targetUserDigestThreads = await db.ThreadModel.extendThreads(
    targetUserDigestThreads,
    {
      forum: true,
      firstPost: true,
      firstPostUser: true,
