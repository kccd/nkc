...
router
  .use('/', OnlyUser(), async (ctx, next) => {
    ...
    if (data.verifyMobile) {
      data.mobile =
        '+' +
        userPersonal.nationCode +
        ' ' +
        userPersonal.mobile.slice(0, 3) +
        '****' +
        userPersonal.mobile.slice(7);
    }
    if (data.verifyEmail) {
      data.email = userPersonal.email.replace(/.{4}@/gi, '****@');
    }
    const { status, passed } = await db.UserModel.checkStatusForDestroyAccount(
      user.uid,
    );
    data.passed = passed;
    data.notices = [];
  ...
  .post('/', OnlyUser(), async (ctx, next) => {
    ...
    const { emailCode, mobileCode, password } = form;
    ...
    if (type === 'destroy') {
      if (mobileCodeObj) {
        await mobileCodeObj.updateOne({ used: true });
      }
      if (emailCodeObj) {
        await emailCodeObj.updateOne({ used: true });
      }
