...
router
  .use('/', Public(), async (ctx, next) => {
    ...
    if (!ctx.permission('modifyAllResource')) {
      if (library.deleted) {
        ctx.throw(404, `数据已被删除`);
      }
      data.nav.map((l) => {
        if (l.closed) {
          ctx.throw(400, `文库已关闭，暂无法访问。`);
        }
      });
    }
    ...
    if (!forum) {
      ctx.throw(
        400,
        `当前文件夹的顶层文件夹不属于任何专业，lid: ${topFolder._id}`,
      );
    }
    // 验证用户是否有权访问顶层文件夹所属专业
  ...
  .get('/', Public(), async (ctx, next) => {
    ...
    if (folder) {
      data.folders = await library.getFolders();
    }
    if (file) {
      data.files = await library.getFiles();
    }
  ...
  .post('/', OnlyUnbannedUser(), async (ctx, next) => {
    ...
    await library.ensurePermission(
      data.user,
      'createFile',
      ctx.permission('modifyAllResource'),
    );

    const { rid, name, category, description } = body;
    const resource = await db.ResourceModel.findOne({ rid });
    if (!resource) {
      ctx.throw(400, `rid异常，数据库中无对应的resource数据。rid: ${rid}`);
    }
  ...
  .put('/', OnlyUnbannedUser(), async (ctx, next) => {
    ...
    if (library.type === 'file') {
      ...
      if (
        category &&
        ['book', 'paper', 'program', 'media', 'other'].includes(category)
      ) {
