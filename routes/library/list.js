...
router
  ...
  .put('/', OnlyUnbannedUser(), async (ctx, next) => {
    ...
    const { library } = data;
    const targetFolder = await db.LibraryModel.findOne({
      _id: targetFolderId,
      type: 'folder',
      deleted: false,
    });
    if (!targetFolderId) {
      ctx.throw(400, `目标目录不正确`);
    }
    const nav = await targetFolder.getNav();
    const navId = nav.map((n) => n._id);
    let ignoreCount = 0;
    for (const _id of foldersId) {
      const folder = await db.LibraryModel.findOne({
        lid: library._id,
        _id,
        deleted: false,
      });
      ...
      const sameName = await db.LibraryModel.countDocuments({
        lid: targetFolderId,
        type: folder.type,
        name: folder.name,
        deleted: false,
  ...
  .del('/', OnlyUnbannedUser(), async (ctx, next) => {
    ...
    for (const id of lid) {
      ...
      const foldersCount = await db.LibraryModel.countDocuments({
        lid: id,
        deleted: false,
      });
      if (foldersCount > 0) {
