...
router
  ...
  .use('/:_id/c/:cid', OnlyUnbannedUser(), async (ctx, next) => {
    const { params, db, data } = ctx;
    const { cid } = params;
    const { user, note } = data;
    const noteContent = await db.NoteContentModel.findOne({ _id: cid });
    if (!noteContent) {
      ctx.throw(400, `笔记不存在，cid: ${noteContent._id}`);
    }
    if (note.originId !== noteContent.noteId) {
      ctx.throw(400, `划词选区与笔记内容无法对应`);
    }
    if (user.uid !== noteContent.uid) {
      ctx.throw(403, '权限不足');
    }
  ...
  .put('/:_id/c/:cid', OnlyUnbannedUser(), async (ctx, next) => {
    // 修改笔记内容
  ...
  .del('/:_id/c/:cid', OnlyUnbannedUser(), async (ctx, next) => {
    // 用户删除
    const { data } = ctx;
    const { noteContent } = data;
  ...
  .post('/', OnlyUnbannedUser(), async (ctx, next) => {
    ...
    if (_id) {
      ...
      if (!note) {
        ctx.throw(400, `笔记ID错误，请重试。id:${_id}`);
      }
    } else if (node) {
