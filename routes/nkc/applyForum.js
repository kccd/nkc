...
router
  .use(
    '/',
    OnlyOperation(Operations.nkcManagementApplyForum),
    async (ctx, next) => {
      const { db, data } = ctx;
      const { user } = data;
  ...
  .get(
    ...
    async (ctx, next) => {
      ...
      const pForums = await db.PreparationForumModel.find({})
        .sort({ toc: -1 })
        .skip(paging.start)
  ...
  .post(
    ...
    async (ctx, next) => {
      ...
      if (agree) {
        // 在正式专业表中创建筹备专业
        const newForum = await db.ForumModel.createForum(
          pForum.info.newForumName,
          'pForum',
        );
        // 把创始人名单添加到记录中
        const agreeUsersId = pForum.founders
          .filter((f) => f.accept === 'resolved')
          .map((f) => f.uid);
