...
router
  .get('/', async (ctx, next) => {
    ...
    if (type === 'today') {
      ...
      for (let i = 0; i < 24; i++) {
        ...
        const usersCount = await db.UserModel.countDocuments({
          toc: { $gt: minTime, $lt: maxTime },
        });
        const postsCount = await db.PostModel.countDocuments({
          toc: { $gt: minTime, $lt: maxTime },
        });
      ...
      data.onlineUsers = [];
      data.onlineUsersCount = await db.UserModel.countDocuments({
        online: { $ne: '' },
      });
      const onlineUsers = await db.UserModel.find({ online: { $ne: '' } })
        .sort({ toc: 1 })
        .limit(5000);
      ...
      // 获取统计
      const keysString = await nkcModules.getRedisKeys(
        'operationStatistics',
        '*',
      );
      const startTimeKey = await nkcModules.getRedisKeys(
        'operationStatisticsStartTime',
      );
      ...
      for (const o of operationArray) {
        ...
        const [operationId, count, time, maxTime, minTime] = result;
  ...
  .use(
    '/applyForum',
    applyForumRouter.routes(),
    applyForumRouter.allowedMethods(),
  )
