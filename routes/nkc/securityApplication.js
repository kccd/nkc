...
router
  .get(
    '/',
    OnlyOperation(Operations.nkcManagementSecurityApplication),
    async (ctx, next) => {
      ...
      const applications = await db.SecurityApplicationModel.find(match)
        .sort({ toc: -1 })
        .skip(paging.start)
  ...
  .post(
    ...
    async (ctx, next) => {
      ...
      const application = await db.SecurityApplicationModel.findById(
        applicationId,
      );
      if (!application) {
        ctx.throw(404, `未找到相关数据 id: ${applicationId}`);
      }
      if (application.status !== 'pending') {
        ctx.throw(400, `当前申诉已被处理，请刷新`);
      }
      if (!['resolved', 'rejected'].includes(status)) {
        ctx.throw(400, `status error`);
      }
      if (status === 'rejected') {
        checkString(reason, {
          name: '理由',
      ...
      if (status === 'resolved') {
        ...
        await db.UsersPersonalModel.modifyVerifyPhoneNumberTime(
          application.uid,
        );
