...
threadRouter
  ...
  .get('/:tid', async (ctx, next) => {
    ...
    for (const f of thread.mainForumsId) {
      const nav = await db.ForumModel.getForumNav(f);
      if (nav.length) {
        data.forumsNav.push(nav);
      }
    }
    // 判断用户是否具有专家权限
    const isModerator = await db.ForumModel.isModerator(
      state.uid,
      thread.mainForumsId,
    );
    // 页面顶部导航
    data.threadNav = await thread.getThreadNav();
    data.collectedCount = await db.ThreadModel.getCollectedCountByTid(
      thread.tid,
    );
    data.isModerator = isModerator;
    ...
    if (!firstPost) {
      ctx.throw(500, `文章数据错误，oc:${thread.oc}`);
    }
    firstPost = await db.PostModel.extendPost(firstPost, extendPostOptions);
    firstPost.t = nkcModules.nkcRender.replaceLink(firstPost.t);
    thread.firstPost = firstPost;
    ...
    // 获取当前文章下回复的总数目
    const count = await db.PostModel.countDocuments(match);
    const paging_ = nkcModules.apiFunction.paging(
      page,
      count,
      pageSettings.threadPostList,
    );
    ...
    for (let postSin of postAll) {
      let onLog = await db.DelPostLogModel.findOne({
        delType: 'toDraft',
        postType: 'post',
        postId: postSin.pid,
        modifyType: false,
    ...
    await db.DelPostLogModel.updateMany(
      {
        delType: 'toDraft',
        postType: 'post',
        threadId: tid,
      ...
      if (voteUpPostSettings.status === 'show') {
        ...
        if (voteUpPostsId.length === postCount) {
          ...
          for (const voteUpPostId of voteUpPostsId) {
            const p = voteUpPostsObj[voteUpPostId];
            if (p) {
              data.voteUpPosts.push(p);
            }
          }
          data.voteUpPosts = await db.PostModel.filterPostsInfo(
            data.voteUpPosts,
          );
        }
      }
      ...
      if (thread.uid === data.user.uid) {
        ...
        if (data.authorColumn) {
          ...
          const columnPost = await db.ColumnPostModel.findOne({
            tid: thread.tid,
            columnId: data.authorColumn._id,
            type: 'thread',
          });
    ...
    if (!data.anonymous) {
      ...
      data.targetUser.description = nkcModules.nkcRender.replaceLink(
      ...
      if (data.targetColumn) {
        data.columnPost = await db.ColumnPostModel.findOne({
          columnId: data.targetColumn._id,
          type: 'thread',
          pid: thread.oc,
        });
      }
      ...
      let targetUserThreads = await db.ThreadModel.find(q)
        .sort({ toc: -1 })
        .limit(10);
      data.targetUserThreads = await db.ThreadModel.extendThreads(
        targetUserThreads,
        {
          forum: true,
          firstPostUser: true,
    ...
    // 【待改】如果当前文章为商品贴
    if (thread.type === 'product') {
      const products = await db.ShopGoodsModel.find({
        tid: thread.tid,
        oc: thread.firstPost.pid,
      });
      ...
      if (data.product.vipDiscount) {
        ...
        for (let v = 0; v < data.product.vipDisGroup.length; v++) {
          if (
            data.user &&
            data.user.authLevel == data.product.vipDisGroup[v].vipLevel
          ) {
      ...
      if (applicationForm) {
        ...
        data.targetUserInFundBlacklist =
          await db.FundBlacklistModel.inBlacklist(applicationForm.uid);
        await data.applicationForm.hideApplicationFormInfoByUserId(
          state.uid,
    ...
    if (data.user) {
      ...
      const vote = await db.PostsVoteModel.findOne({
        source: postSource,
        uid: data.user.uid,
        sid: thread.oc,
      });
      ...
      data.redEnvelopeSettings = await db.SettingModel.getSettings(
        'redEnvelope',
      );
    }
    // 设置标志 是否关注，是否收藏
    ...
    data.homeTopped = await db.SettingModel.isEqualOfArr(
      data.homeSettings.toppedThreadsId,
      { id: tid, type: 'thread' },
    );
    data.latestTopped = await db.SettingModel.isEqualOfArr(
      data.homeSettings.latestToppedThreadsId,
      { id: tid, type: 'thread' },
    );
    data.communityTopped = await db.SettingModel.isEqualOfArr(
      data.homeSettings.communityToppedThreadsId,
      { id: tid, type: 'thread' },
    );
    if (thread.type === 'product' && ctx.permission('pushGoodsToHome')) {
      data.goodsHomeTopped = data.homeSettings.shopGoodsId.includes(
    ...
    if (data.user) {
      ...
      data.hasPermissionToHidePost =
        await db.PostModel.ensureHidePostPermission(data.thread, data.user);
    ...
    if (
      ...
      const allPosts = await db.PostModel.find(
        { tid: data.thread.tid },
        { pid: 1 },
      );
      const pid = allPosts.map((p) => p.pid);
      data.attachmentsCount = await db.ResourceModel.countDocuments({
        mediaType: 'mediaAttachment',
        references: { $in: pid },
      });
    }
