...
router.get('/', OnlyUnbannedUser(), async (ctx, next) => {
  ...
  if (type === 'UTU') {
    ...
    const friend = await db.FriendModel.findOne(
      { uid: user.uid, tUid: targetUser.uid },
      { info: 1 },
    );
    const name =
      friend && friend.info.name
        ? friend.info.name
    ...
    const chat = await db.CreatedChatModel.findOne({
      uid: user.uid,
      tUid: targetUser.uid,
    });
    if (!chat) {
      await ctx.nkcModules.socket.sendEventUpdateChat(
        'UTU',
        user.uid,
        targetUser.uid,
      );
    ...
    const messages = await db.MessageModel.getUserSystemInfoMessages(
      user.uid,
      queryDoc,
    );
    // 取纯文本返回给前端
    ...
    const friendsApplications = await db.FriendsApplicationModel.find(q)
      .sort({ toc: -1 })
      .limit(30);
    const applications = [];
    for (const f of friendsApplications) {
      const applicationMessage =
        await db.FriendsApplicationModel.getApplicationMessage(f._id);
