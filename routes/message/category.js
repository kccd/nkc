...
router
  .get('/', OnlyUnbannedUser(), async (ctx, next) => {
    ...
    if (cid) {
      // 查看、编辑分组
      data.category = await db.FriendsCategoryModel.findOne(
        { _id: cid, uid: user.uid },
        {
          _id: 1,
          name: 1,
          description: 1,
    ...
    const friends = await db.FriendModel.find(
      { uid: user.uid },
      { tUid: 1, cid: 1 },
    );
  ...
  .del('/', OnlyUnbannedUser(), async (ctx, next) => {
    ...
    const { user } = data;
    const { cid } = query;
    const category = await db.FriendsCategoryModel.findOne({
      uid: user.uid,
      _id: cid,
    });
    if (category) {
