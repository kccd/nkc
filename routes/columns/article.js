...
router.get('/:aid', Public(), async (ctx, next) => {
  ...
  let columnPostData = await db.ColumnPostModel.getDataRequiredForArticle(
    _id,
    aid,
    xsf,
  );
  ...
  if (columnPostData.type === article) {
    ...
    //文章收藏数
    data.columnPost.collectedCount =
      await db.ThreadModel.getCollectedCountByTid(thread.tid);
    //获取用户是否收藏文章
    if (user) {
      ...
      //   data.columnPost.collected = true;
      // }
      data.columnPost.collected = await collectionService.isCollectedThread(
        user.uid,
        thread.tid,
      );
    }
