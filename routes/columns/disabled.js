...
router.post(
  '/',
  OnlyOperation(Operations.column_single_disabled),
  async (ctx, next) => {
    ...
    if (type === 'column') {
      ...
      if (type === 'name') {
        ...
        if (column.noticeDisabled && disabled)
          ctx.throw(400, '公告通知已被屏蔽，请刷新');
        obj.noticeDisabled = disabled;
      } else if (type === 'blocks') {
        if (column.blocksDisabled && disabled)
          ctx.throw(400, '自定义内容已被屏蔽，请刷新');
        obj.blocksDisabled = disabled;
      } else if (type === 'otherLinks') {
        if (column.otherLinksDisabled && disabled)
          ctx.throw(400, '友情链接已被屏蔽，请刷新');
