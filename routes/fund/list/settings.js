...
settingsRouter
  // 具体某个基金项目设置页面
  .get('/', OnlyOperation(Operations.singleFundSettings), async (ctx, next) => {
    const { data, db } = ctx;
    ...
    const { fund } = data;
    data.roles = await db.RoleModel.find({}, { _id: 1, displayName: 1 }).sort({
      toc: 1,
    });
  ...
  .put('/', OnlyOperation(Operations.singleFundSettings), async (ctx, next) => {
