...
router
  ...
  //   if (!fundObj.applicationMethod.personal && !fundObj.applicationMethod.team)
  //     ctx.throw(400, '必须勾选申请方式。');
  //   if (
  //     fundObj.auditType === 'system' &&
  //     fundObj.admin.appointed.length === 0
  //   ) {
  ...
  .get('/', Public(), async (ctx, next) => {
    ...
    if (token) {
      ...
      let fundlistShareLimit = await db.ShareLimitModel.findOne({
        shareType: 'fundlist',
      });
      ...
      if (share.shareUrl.indexOf(ctx.path) === -1)
        ctx.throw(403, '无效的token');
    }

    const fund = await db.FundModel.findOnly({
      _id: fundId.toUpperCase(),
      disabled: false,
    });
    data.fund = fund;
    let query = {
    ...
    const applicationForms = await FundApplicationFormModel.find(query)
      .sort(sortObj)
      .skip(paging.start)
      .limit(paging.perpage);
    data.applicationForms =
      await db.FundApplicationFormModel.extendAsApplicationFormList(
        applicationForms,
      );
    data.paging = paging;
    //改由前端判断
    if (data.user) {
      data.conditions = await db.FundModel.getConditionsOfApplication(
        user.uid,
        fund._id,
      );
    }
