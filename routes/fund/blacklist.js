...
router
  .get('/', OnlyUser(), async (ctx, next) => {
    ...
    const list = await db.FundBlacklistModel.find({})
      .sort({ toc: -1 })
      .skip(paging.start)
    ...
    const applicationForms = await db.FundApplicationFormModel.find(
      { _id: { $in: applicationFormsId } },
      { code: 1, _id: 1 },
    );
    applicationForms.map((a) => (applicationFormsObj[a._id] = a));
    for (const l of list) {
      const { toc, uid, reason, operatorUid, applicationFormId } = l;
      const u = usersObj[uid];
      const operator = usersObj[operatorUid];
      const applicationForm = applicationFormId
        ? applicationFormsObj[applicationFormId]
        : null;
      data.list.push({
        toc,
        user: u,
  ...
  .post('/', OnlyOperation(Operations.fundBlacklistPost), async (ctx, next) => {
  ...
  .del('/', OnlyOperation(Operations.fundBlacklistPost), async (ctx, next) => {
    const { db, query } = ctx;
