...
settingsRouter
  ...
  .get('/', OnlyUser(), async (ctx, next) => {
    ...
    const posts = await db.PostModel.find({
      type: 'thread',
      tid: { $in: applicationForm.threadsId.applying },
    });
    const forums = await db.ForumModel.find(
      { fid: applicationForm.category },
      { fid: 1, displayName: 1 },
    );
  ...
  .post('/', OnlyUnbannedUser(), async (ctx, next) => {
    ...
    const { name, idCardNumber, mobile, description } = applicant;
    const { t, c, l, abstractEn, abstractCn, keyWordsEn, keyWordsCn } = project;
    const { paymentType, bankName, name: accountName, number } = account;

    const _budgetMoney = [];

    for (let i = 0; i < budgetMoney.length; i++) {
      const { count, money, purpose, unit = '', model = '' } = budgetMoney[i];
    ...
    const deletedLifePhotosId = oldLifePhotosId.filter(
      (id) => !lifePhotosId.includes(id),
    );
    // 将生活照复制一份到基金申请
    lifePhotosId = await db.PhotoModel.copyLifePhotosToFund(
      applicationForm.uid,
      applicationForm._id,
      lifePhotosId,
    );
    ...
    if (isSubmit) {
      ...
      const agreeMembersId =
        await db.FundApplicationUserModel.getAgreeMembersId(
          applicationForm._id,
        );
      const applicationFormUsersId = [applicationForm.uid].concat(
        agreeMembersId,
      );
      const posts = await db.PostModel.find(
        {
      ...
      if (posts.length !== threadsId.applying.length)
        ctx.throw(400, `技术文章 ID 错误，请重新选择`);
      ...
      const formForum = await db.ForumModel.findOne(
        { fid: category },
        { fid: 1 },
      );
      ...
      // 删除被移除的基金生活照
      await db.PhotoModel.removeApplicationFormPhotosById(
        applicationForm.uid,
        applicationForm._id,
        deletedLifePhotosId,
      );
    ...
    // 更新申请表
    await db.FundApplicationFormModel.updateOne(
    ...
    if (isSubmit) {
      // 更新申请表总金额
      await db.FundApplicationFormModel.updateMoneyByApplicationFormId(
        applicationForm._id,
      );
      // 发布申请
      await db.FundApplicationFormModel.publishByApplicationFormId(
        applicationForm._id,
      );
    } else {
