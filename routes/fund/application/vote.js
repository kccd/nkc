...
voteRouter.post('/', OnlyUnbannedUser(), async (ctx, next) => {
  ...
  if (!(await applicationForm.fund.ensureOperatorPermission('voter', user))) {
    ctx.throw(403, '抱歉！您没有资格进行投票。');
  }
  const { fund, members, supportersId, objectorsId, status } = applicationForm;
  if (!status.submitted) {
    ctx.throw(400, '申请表未提交，暂不能投票。');
  }
  const membersId = members.map((m) => m.uid);
  membersId.push(applicationForm.uid);
  if (membersId.includes(user.uid)) {
    ctx.throw(403, '抱歉！您已参与该基金的申请，无法完成该操作！');
  }
  if (supportersId.includes(user.uid) || objectorsId.includes(user.uid)) {
    ctx.throw(400, '抱歉！您已经投过票了。');
  }
  if (type === 'support') {
  ...
  //获得网友支持
  if (
    !applicationForm.status.usersSupport &&
    fund.supportCount <= supportersId.length
  ) {
    const obj = {
