...
completeRouter
  ...
  .get('/', async (ctx, next) => {
    ...
    if (status.completed === false) {
      ...
      data.completedReport = await db.FundDocumentModel.findOne({
        applicationFormId: applicationForm._id,
  ...
  .post('/', async (ctx, next) => {
    ...
    if (!fixedMoney) {
      for (let i = 0; i < actualMoney.length; i++) {
        const { purpose, model = '', money, count, unit = '' } = actualMoney[i];
