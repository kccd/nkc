...
billsRouter
  .get('/', Public(), async (ctx, next) => {
    ...
    bills = bills.reverse();
    data.balance = total;
    const targetBills = bills.slice(
      paging.start,
      paging.start + paging.perpage,
    );
    data.bills = await Promise.all(
      targetBills.map(async (b) => {
        await b.extendUser();
  ...
  .post('/', OnlyOperation(Operations.addFundBill), async (ctx, next) => {
    ...
    //来自用户且匿名为否 未输入uid
    if (to.type === 'user' && !to.id && !to.anonymous)
      ctx.throw(400, '请输入用户UID。');
    if (from.type === 'user' && !from.id && !from.anonymous)
      ctx.throw(400, '请输入用户UID。');

    if (from.type === 'user') {
      if (!['fundPool', 'fund'].includes(to.type))
        ctx.throw(
          400,
          '用户可执行的操作有：赞助给基金项目、赞助给资金池、退还剩余款项。',
        );
      ...
      if (to.type === 'fund') {
        if (fund._id === to.id)
          ctx.throw(400, '资金来源与作用的基金项目不能相同。');
  ...
  .use(
    '/:billId',
    OnlyOperation(Operations.visitFundBill),
    async (ctx, next) => {
  ...
  .get(
    ...
    async (ctx, next) => {
      ...
      ctx.data.funds = await ctx.db.FundModel.find({
        disabled: false,
        history: false,
      }).sort({ toc: 1 });
      await next();
  ...
  .del(
    '/:billId',
    OnlyOperation(Operations.deleteFundBill),
    async (ctx, next) => {
  ...
  .put(
    '/:billId',
    OnlyOperation(Operations.modifyFundBill),
    async (ctx, next) => {
      const { body, data } = ctx;
