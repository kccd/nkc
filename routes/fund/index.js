...
fundRouter
  ...
  .get('/', Public(), async (ctx, next) => {
    ...
    const applying = await db.FundApplicationFormModel.find(queryOfApplying)
      .sort({ toc: -1 })
      .limit(10);
    data.applying =
      await db.FundApplicationFormModel.extendAsApplicationFormList(applying);
    const funding = await db.FundApplicationFormModel.find(queryOfFunding)
      .sort({ toc: -1 })
      .limit(10);
    data.funding =
      await db.FundApplicationFormModel.extendAsApplicationFormList(funding);
    const excellent = await db.FundApplicationFormModel.find(
      queryOfExcellent,
    ).sort({ toc: 1 });
    data.excellent =
      await db.FundApplicationFormModel.extendAsApplicationFormList(excellent);
    const completed = await db.FundApplicationFormModel.find(
      queryOfCompleted,
    ).sort({ timeOfCompleted: -1 });
    data.completed =
      await db.FundApplicationFormModel.extendAsApplicationFormList(completed);
