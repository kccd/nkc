...
router
  .use('/', Public(), async (ctx, next) => {
    ...
    if (!survey) {
      ctx.throw(400, `未找到ID为${surveyId}的表单`);
    }
    if (survey.disabled) {
      ctx.throw(403, '表单已屏蔽');
    }
  ...
  .get('/', Public(), async (ctx, next) => {
    ...
    if (
      ...
      (survey.showVoter === 'after' &&
        Date.now() > new Date(survey.et).getTime()) // 结束后显示
    ) {
      data.users = await survey.getPostUsers();
    }
    data.allowedUsers = await db.UserModel.find({
      uid: { $in: survey.permission.uid },
    });
    data.targetUser = await db.UserModel.findOnly({ uid: survey.uid });
    data.surveyRewardScore = await db.SettingModel.getScoreByOperationType(
      'surveyRewardScore',
    );
    if (data.user) {
      data.targetUserSurveyRewardScore = await db.UserModel.getUserScore(
        data.user.uid,
        data.surveyRewardScore.type,
      );
      data.surveyPost = await db.SurveyPostModel.findOne({
        uid: data.user.uid,
        surveyId: survey._id,
      });
      try {
    ...
    if (data.user && data.user.uid === survey.uid) {
      // 发起人永远可见
      data.showResult = true;
    } else if (ctx.permission('showSecretSurvey')) {
      // 管理员永远可见
      data.showResult = true;
    } else if (survey.showResultAfterTheEnd) {
      // 发起人设定结束后可见
      if (Date.now() >= new Date(survey.et).getTime()) {
        // 已结束
        if (survey.showResult === 'posted' && data.surveyPost) {
          // 提交后可见且已提交
          data.showResult = true;
        } else if (survey.showResult === 'all') {
          // 所有人可见
          data.showResult = true;
        }
      }
    } else {
      // 未设置结束可见，所以无需判断当前是否已结束
      if (survey.showResult === 'posted' && data.surveyPost) {
        // 提交后可见且已提交
        data.showResult = true;
      } else if (survey.showResult === 'all') {
  ...
  .post('/', Public(), async (ctx, next) => {
    ...
    if (type === 'newPost') {
      ...
      surveyPost = await survey.ensureModifyPostPermission(
        user ? user.uid : '',
        ctx.address,
      );
    }
    ...
    if (survey.type !== 'score') {
      for (const key in selectedCount) {
        if (!selectedCount.hasOwnProperty(key)) {
          continue;
        }
        const sc = selectedCount[key];
        if (sc.selectedCount < sc.minVoteCount) {
          ctx.throw(400, '勾选选项的数目未达最低要求，请检查');
        }
      }
    }
    ...
    if (type === 'newPost') {
      ...
      surveyPostDB._id = await db.SettingModel.operateSystemID(
        'surveyPosts',
        1,
      );
