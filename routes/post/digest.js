...
router
  .post('/', OnlyOperation(Operations.digestPost), async (ctx, next) => {
    //文章和回复加精，加科创币
    ...
    const redEnvelopeSettings = await db.SettingModel.findOnly({
      _id: 'redEnvelope',
    });
    let num;
    if (!redEnvelopeSettings.c.draftFee.close) {
      ...
      if (
        !redEnvelopeSettings.c.draftFee.close &&
        (num < redEnvelopeSettings.c.draftFee.minCount ||
          num > redEnvelopeSettings.c.draftFee.maxCount)
      ) {
    ...
    if (thread.oc === pid) {
      ...
      //   await record.save();
      // }
      await db.PostModel.insertSystemRecord(
        'digestThread',
        data.targetUser,
        ctx,
        num,
  ...
  .del('/', OnlyOperation(Operations.unDigestPost), async (ctx, next) => {
    //文章和回复取消加精，扣除科创比
