...
router.get('/', Public(), async (ctx, next) => {
  ...
  // 是否可以发表评论
  data.cWriteInfo = (await db.PostModel.checkPostCommentPermission(
    pid,
    'write',
  ))
    ? null
  ...
  let parentPosts = await db.PostModel.find(q)
    .sort({ toc: 1 })
    .skip(paging.start)
  ...
  data.sendAnonymousPost = await db.UserModel.havePermissionToSendAnonymousPost(
    'postToThread',
    state.uid,
    post.mainForumsId,
  );
