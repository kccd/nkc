    ...
    .get('/:pid', Public(), async(ctx, next) => {
        ...
        if (notices.length !== 0) {
            ...
            if (
                ...
                postHistory = !post.hideHistories || ctx.permission('displayPostHideHistories') ?
                    true :
                    null;
            }
            const userId = Array.from(new Set(notices.map((item) => item.uid)));
            //获取公告用户信息
            const users = await db.UserModel.find({ uid: { $in: userId } }, { avatar: 1, uid: 1, username: 1 }, ).lean();
            //获取历史版本
            const cv = Array.from(
                new Set(notices.map((item) => item.cv).filter(Boolean)),
            );
            //获取hid数组对象
            const hidArr = await db.HistoriesModel.find({ pid, cv: { $in: cv } }, { _id: 1, cv: 1 }, ).lean();
            //筛选出来的hid对象
    ...
    .put('/:pid', OnlyUnbannedUser(), async(ctx, next) => {
        ...
        if (columnMainCategoriesId.length > 0 && userColumn) {
            await db.ColumnPostModel.addColumnPosts(
                userColumn,
                columnMainCategoriesId,
                columnMinorCategoriesId, [targetThread.oc],
            );
        }
