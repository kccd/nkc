...
router.get('/', Public(), async (ctx, next) => {
  ...
  if (user) {
    ...
    if (userColumn && isThread) {
      ...
      optionStatus.inColumn = await db.ColumnPostModel.checkColumnPost(
        userColumn._id,
        pid,
      );
    }
    ...
    if (isPost) {
      ...
      if (
        ...
        (ctx.permission('topAllPost') || // 特殊指定权限
          (user.uid === thread.uid &&
            (await db.PostModel.ensureToppingPermission(user.uid)))) // 文章作者且具备置顶的条件
      ) {
      ...
      if (
        !isComment &&
        (await db.PostModel.ensureHidePostPermission(thread, user))
      ) {
    ...
    if (
      ...
      const modifyPostTimeLimit = await db.UserModel.getModifyPostTimeLimitMS(
        user.uid,
      );
