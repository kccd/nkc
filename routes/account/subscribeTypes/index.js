...
router
  .get('/', async (ctx, next) => {
    ...
    if (count) {
      data.counts = {
        total: await db.SubscribeModel.countDocuments({
          cancel: false,
          uid: user.uid,
        }),
        other: await db.SubscribeModel.countDocuments({
          cancel: false,
          uid: user.uid,
          cid: [],
        }),
      ...
      data.counts.topic = await db.SubscribeModel.countDocuments({
        cancel: false,
        uid: user.uid,
        source: subscribeSources.forum,
        sid: { $in: topic },
      });
      data.counts.discipline = await db.SubscribeModel.countDocuments({
        cancel: false,
        uid: user.uid,
        source: subscribeSources.forum,
        sid: { $in: discipline },
  ...
  .post('/', async (ctx, next) => {
    ...
    const lastType = await db.SubscribeTypeModel.findOne({
      uid: user.uid,
    }).sort({ order: -1 });
