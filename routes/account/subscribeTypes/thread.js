...
router
  .use('/', async (ctx, next) => {
    ...
    if (!type) {
      ctx.throw(400, `未找到ID为${tid}的关注分类`);
    }
    if (type.type) {
      ctx.throw(400, `默认分类无法更改`);
    }
  ...
  .put('/', async (ctx, next) => {
    ...
    if (type === 'info') {
      ...
      if (!name) {
        ctx.throw(400, '分类名不能为空');
      }
      if (contentLength(name) > 20) {
        ctx.throw(400, '分类名不能超过20字符');
      }
      if (pid) {
        const parentType = await db.SubscribeTypeModel.findOne({
          uid: user.uid,
          _id: pid,
        });
        if (!parentType) {
          ctx.throw(400, `未找到ID为${pid}的关注分类`);
        }
        const childTypesCount = await db.SubscribeTypeModel.countDocuments({
          uid: user.uid,
          pid: subscribeType._id,
        });
        if (childTypesCount) {
          ctx.throw(400, '该分类下存在子分类，无法将该分类设置成子分类');
        }
      }
      const sameName = await db.SubscribeTypeModel.findOne({
        name,
        uid: user.uid,
        _id: { $ne: subscribeType._id },
      });
      if (sameName) {
      ...
      if (subscribeType.pid) {
        types = await db.SubscribeTypeModel.find({
          uid: user.uid,
          pid: subscribeType.pid,
        }).sort({ order: 1 });
      } else {
        types = await db.SubscribeTypeModel.find({
          uid: user.uid,
      ...
      for (let i = 0; i < typesId.length; i++) {
        var _id = typesId[i];
        await db.SubscribeTypeModel.updateOne(
          { _id },
          {
            $set: {
              order: i,
  ...
  .del('/', async (ctx, next) => {
    ...
    if (childTypesCount) {
      ctx.throw(400, '分类下存在子分类，无法删除');
    }
    const subCount = await db.SubscribeModel.countDocuments({
      cancel: false,
      uid: user.uid,
      cid: subscribeType._id,
    });
    if (subCount) {
      ctx.throw(400, '分类下存在关注的内容，无法删除');
    }
