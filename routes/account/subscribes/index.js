...
router
  .get('/', OnlyUnbannedUser(), async (ctx, next) => {
    ...
    if (
      ...
      if (c !== 'all') {
        if (['topic', 'discipline'].includes(c)) {
          q.source = subscribeSources.forum;
          const forums = await db.ForumModel.find({ forumType: c }, { fid: 1 });
          q.sid = { $in: forums.map((f) => f.fid) };
        } else {
          q.source = c;
        }
      }
    ...
    const subscribes = await db.SubscribeModel.find(q)
      .sort({ toc: -1 })
      .skip(paging.start)
      .limit(paging.perpage);
    data.subscribes = await db.SubscribeModel.extendSubscribes(subscribes);
    data.subForumsId = await db.SubscribeModel.getUserSubForumsId(
      data.user.uid,
    );
    ...
    data.collectionThreadsId =
      await db.SubscribeModel.getUserCollectionThreadsId(user.uid);
    data.paging = paging;
    await next();
  })
  .put('/', OnlyUnbannedUser(), async (ctx, next) => {
    ...
    if (type === 'modifyType') {
      ...
      for (const typeId of typesId) {
        ...
        if (!type) {
          ctx.throw(400, `未找到ID为${typeId}的分类`);
        }
