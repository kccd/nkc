...
router
  ...
  .post('/', OnlyUnbannedUser(), async (ctx, next) => {
    ...
    try {
      ...
      const rechargeSettings = await db.SettingModel.getSettings('recharge');
      const { enabled, min, max, startingTime, endTime, countOneDay } =
        rechargeSettings.withdraw;
      ...
      if (countToday >= countOneDay)
        ctx.throw(403, '你今日的提现次数已用完，请明天再试');
      const _effectiveMoney = Math.floor(money * (1 - payment.fee));
      if (_effectiveMoney !== effectiveMoney)
        ctx.throw(400, `页面数据已过期，请刷新后再试`);
      if (effectiveMoney < min)
        ctx.throw(400, `提现金额不能小于 ${min / 100} 元`);
      if (effectiveMoney > max)
        ctx.throw(400, `提现金额不能超过 ${max / 100} 元`);
      checkNumber(money, {
        name: '提现金额',
        min: 1,
      });
      ...
      if (selectPayment === 'aliPay') {
        ...
        for (const a of alipayAccounts) {
          if (a.account === account.account && a.name === account.name)
            existing = true;
        }
