...
router
  .post('/', OnlyOperation(Operations.digestArticle), async (ctx, next) => {
    //独立文章加精
    const { db, data, query, params, state, permission, nkcModules, body } =
      ctx;
    const { aid } = params;
    const { kcb } = body;
    let messageType = 'digestArticle';
    if (!permission('digestArticle')) return ctx.throw(401, '权限不足');
    const digestRewardScore = await db.SettingModel.getScoreByOperationType(
      'digestRewardScore',
    );
    ...
    const redEnvelopeSetting = await db.SettingModel.findOnly({
      _id: 'redEnvelope',
    });
    let num;
    if (!redEnvelopeSetting.c.draftFee.close) {
      ...
      num = Number(kcb);
      if (num % 1 !== 0)
        ctx.throw(400, `${digestRewardScore.name}仅支持到小数点后两位`);
      if (
        !redEnvelopeSetting.c.draftFee.close &&
        (num < redEnvelopeSetting.c.draftFee.minCount ||
          num > redEnvelopeSetting.c.draftFee.maxCount)
      )
        ctx.throw(400, `${digestRewardScore.name}数额不在范围内`);
    }

    const usersGeneralSettings = await db.UsersGeneralModel.findOnly({
      uid: _article.user.uid,
    });
    ...
    await message.save();
    if (!redEnvelopeSetting.c.draftFee.close) {
      await usersGeneralSettings.updateOne({
        $inc: { 'draftFeeSettings.kcb': num },
      });
    }
  ...
  .del('/', OnlyOperation(Operations.unDigestArticle), async (ctx, next) => {
    ...
    const rewardLog = await db.KcbsRecordModel.findOne({
      type: 'digestArticle',
      pid: aid,
    }).sort({ toc: -1 });
    if (rewardLog) {
