...
router.post('/', OnlyOperation(Operations.moveThreads), async (ctx, next) => {
  ...
  for (const thread of threads) {
    ...
    if (!isModerator) {
      ctx.throw(403, `您没有权限处理ID为 ${thread.tid} 的文章`);
    }
  }
  ...
  for (const thread of threads) {
    ...
    if (moveType === 'add') {
      ...
      if (mainForumsId.includes(recycleId)) {
        ctx.throw(403, `无法给回收站中的文章添加专业`);
      }
      // 排除掉已选专业中未被选择的文章分类
    ...
    if (remindUser) {
      const newThread = await db.ThreadModel.findOnly(
        { tid: thread.tid },
        { mainForumsId: 1 },
      );
