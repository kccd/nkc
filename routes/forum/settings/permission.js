...
permissionRouter
  .get(
    '/',
    OnlyOperation(Operations.visitForumPermissionSettings),
    async (ctx, next) => {
      const { data, db } = ctx;
      const { forum } = data;
      ...
      if (ctx.permission('createForumLibrary')) {
        if (data.forum.lid) {
          const library = await db.LibraryModel.findOne({
            _id: data.forum.lid,
          });
          if (!library) {
            data.forum.lid = '';
  ...
  .put(
    `/`,
    OnlyOperation(Operations.modifyForumPermission),
    async (ctx, next) => {
      ...
      for (const uid of moderators) {
        ...
        if (!u) {
          ctx.throw(400, `主管专家ID错误 uid: ${uid}`);
        }
      }
      ...
      writePost.gradesId = writePost.gradesId.filter((g) =>
        gradesId.includes(g),
      );
      const relations = [`and`, 'or'];
      if (
        !relations.includes(read.relation) ||
        !relations.includes(write.relation) ||
        !relations.includes(writePost.relation)
      ) {
        ctx.throw(400, '请选择证书等级关系');
      }
      if (
        !Object.values(articlePanelStyleTypes).includes(threadListStyle.type)
      ) {
        ctx.throw(
          400,
          `文章列表显示模式设置错误 type: ${threadListStyle.type}`,
        );
      }
      if (
        ...
        ctx.throw(
          400,
          `文章列表封面图设置错误 cover: ${threadListStyle.cover}`,
        );
      }
        ...
        { fid: forum.fid },
        {
          $set: {
            ...
            disablePost: {
              ...
              gradesId: disablePost.gradesId.filter((id) =>
                gradesId.includes(id),
              ),
      ...
      const oldUserId = oldModerators.filter(
        (uid) => !moderators.includes(uid),
      );
      const newUserId = moderators.filter(
        (uid) => !oldModerators.includes(uid),
      );
