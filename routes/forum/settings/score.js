...
router
  .get('/', OnlyOperation(Operations.forumScoreSettings), async (ctx, next) => {
    const { db, data } = ctx;
    const { forum } = data;
    data.forumAvailableScoreOperations =
      await db.SettingModel.getForumAvailableScoreOperations();
    data.forumScoreOperations = await db.SettingModel.getForumScoreOperations(
      forum.fid,
    );
  ...
  .put('/', OnlyOperation(Operations.forumScoreSettings), async (ctx, next) => {
    ...
    const forumAvailableScoreOperations =
      await db.SettingModel.getForumAvailableScoreOperations();
    const FSOTypes = forumAvailableScoreOperations.map((s) => s.type);
    const _forumScoreOperations = await db.SettingModel.getForumScoreOperations(
      forum.fid,
    );
    ...
    for (const scoreOperation of forumScoreOperations) {
      ...
      if (!FSOTypes.includes(type))
        ctx.throw(400, `未知的积分策略操作类型 type: ${type}`);
      ...
      if (forumScoreOperation) {
        ...
        _operation._id = await db.SettingModel.operateSystemID(
          'scoreOperations',
          1,
        );
