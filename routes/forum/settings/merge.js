...
mergeRouter
  .get(
    '/',
    OnlyOperation(Operations.visitForumMergeSettings),
    async (ctx, next) => {
  ...
  .put(
    '/',
    OnlyOperation(Operations.modifyMergeSettings),
    async (ctx, next) => {
      ...
      let currentThreadType = await db.ThreadTypeModel.findOne({
        fid: targetForum.fid,
        name: forum.displayName,
      });
      if (!currentThreadType) {
        const lastType = await db.ThreadTypeModel.findOne({
          fid: targetForum.fid,
        }).sort({ order: -1 });
      ...
      const forumThreadTypes = await db.ThreadTypeModel.find(
        { fid: forum.fid },
        { cid: 1 },
      );
