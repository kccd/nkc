...
categoryRouter
  .get(
    '/',
    OnlyOperation(Operations.visitForumCategorySettings),
    async (ctx, next) => {
      const { data, db } = ctx;
      const { forum } = data;
      const parentForums = [];
  ...
  .put(
    '/',
    OnlyOperation(Operations.modifyForumCategory),
    async (ctx, next) => {
      ...
      if (operation === 'savePosition') {
        ...
        const forumCategory = await db.ForumCategoryModel.findOne({
          _id: categoryId,
        });
        ...
        const threadType = await db.ThreadTypeModel.findOne({
          name: oldName,
          fid: forum.fid,
        });
  ...
  .post('/', OnlyOperation(Operations.addForumCategory), async (ctx, next) => {
    const { data, db, body, redis } = ctx;
    const { forum } = data;
    const { name } = body;
    if (!name) ctx.throw(400, '分类名不能为空');
    const saveNameType = await db.ThreadTypeModel.findOne({
      fid: forum.fid,
      name,
    });
    if (saveNameType) ctx.throw(400, '分类名已存在');
    const lastType = await db.ThreadTypeModel.findOne({ fid: forum.fid }).sort({
      order: -1,
    });
    let order = 1;
    if (lastType) {
  ...
  .del(
    '/',
    OnlyOperation(Operations.removeForumCategory),
    async (ctx, next) => {
      ...
      await db.ThreadModel.updateMany(
        { categoriesId: threadType.cid },
        { $pull: { categoriesId: threadType.cid } },
      );
