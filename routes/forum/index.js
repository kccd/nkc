...
forumRouter
  ...
  .get('/', async (ctx, next) => {
    ...
    if (t === 'selector') {
      if (!['writable', 'readable'].includes(f)) {
        ctx.throw(400, `专业类型错误 from: ${f}`);
      }
      data.forums = await db.ForumModel.getForumSelectorForums(
        user ? user.uid : '',
        f,
      );
      data.subscribeForumsId = [];
      if (user) {
        data.subscribeForumsId = await db.SubscribeModel.getUserSubForumsId(
          user.uid,
        );
      }
    } else {
      ...
      const forumsOrigin = await db.ForumModel.visibleForums(
        data.userRoles,
        data.userGrade,
        data.user,
      );
      ...
      data.recommendForums = await db.ForumModel.getRecommendForums(
        forumsOrigin.map((f) => f.fid),
      );
  ...
  .post('/', async (ctx, next) => {
    ...
    data.forum = newForum;
    data.forums = await db.ForumModel.find({ parentsId: [] }).sort({
      order: 1,
    });
    await next();
  })
