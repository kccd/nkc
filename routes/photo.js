...
const {
  ...
  lifePhotoCount,
  certPhotoCount,
} = require('../settings/index').upload;

const { photoify, photoSmallify, lifePhotoify } =
  require('../tools').imageMagick;
const { OnlyUnbannedUser, Public } = require('../middlewares/permission');
photoRouter
  .get('/:photoId', Public(), async (ctx, next) => {
  ...
  .post('/', OnlyUnbannedUser(), async (ctx, next) => {
    ...
    const photoTypes = ['idCardA', 'idCardB', 'handheldIdCard', 'cert', 'life'];
    ...
    if (size > sizeLimit.photo) {
      ctx.throw(400, '图片大小不能超过20M！');
    }
    ...
    if ((photoType === 'life' || photoType === 'cert') && size > 1024 * 500) {
  ...
  .del('/:photoId', OnlyUnbannedUser(), async (ctx, next) => {
    ...
    if (
      user.uid !== photo.uid &&
      data.userOperationsId.includes('removeAnyBodyPhoto')
    ) {
      ctx.throw(403, '权限不足');
    }
