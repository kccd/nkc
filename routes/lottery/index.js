...
luckRouter
  .get('/', async (ctx, next) => {
    ...
    const postCount = await db.PostModel.countDocuments({
      uid: user.uid,
      toc: { $gt: today },
    });
    if (postCount !== 1) {
  ...
  .post('/', async (ctx, next) => {
    ...
    try {
      ...
      const postRewardScore = await db.SettingModel.getScoreByOperationType(
        'postRewardScore',
      );
      const { random } = redEnvelopeSettings;
      if (random.close) {
        ctx.throw(403, '抱歉，红包功能已关闭！');
      }
      ...
      if (!generalSettings.lotterySettings.status) {
        ctx.throw(403, '抱歉，你暂未获得开红包机会，请刷新。');
      }
      let n = 1;
