...
router
  ...
  .get('/:rid', async (ctx, next) => {
    ...
    const { rid } = params;
    const { t } = query;
    let sticker = await db.StickerModel.findOne({ from: 'upload', rid });
    const stickerSettings = await db.SettingModel.getSettings('sticker');
    ...
    if (t === 'json') {
      ...
      if (sticker.disabled && !ctx.permission('nkcManagementSticker')) {
        ctx.throw(403, '表情已被屏蔽');
      }
      sticker = sticker.toObject();
      sticker.targetUser = await db.UserModel.findOne(
        { uid: sticker.tUid },
        { username: 1, avatar: 1, uid: 1 },
      );
      if (data.user) {
        sticker.collected = await db.StickerModel.getCollectSticker(
          sticker.rid,
          data.user.uid,
        );
      }
      ...
      if (!sticker) {
        ctx.filePath = statics.lostStickerImage;
      } else if (sticker.disabled && !ctx.permission('nkcManagementSticker')) {
        ctx.filePath = statics.disabledStickerImage;
      } else {
        const resource = await db.ResourceModel.findOnly({
          rid,
          type: 'sticker',
