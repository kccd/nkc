...
sendMessageRouter
  ...
  .post('/getback', async (ctx, next) => {
    ...
    if (!userPersonal.mobile) {
      ctx.throw(400, '此账号未绑定手机号码。');
    }
    if (!nationCode) {
      ctx.throw(400, '请输入国际区号。');
    }
    if (!mobile) {
      ctx.throw(400, '请输入手机号码。');
    }
    ...
    if (
      userPersonal.mobile !== mobile ||
      userPersonal.nationCode !== nationCode
    ) {
  ...
  .post('/bindMobile', async (ctx, next) => {
    ...
    if (userPersonal.mobile && userPersonal.nationCode) {
      ctx.throw(
        400,
        `您的账号已绑定手机号码：+${userPersonal.nationCode} ${userPersonal.mobile}`,
      );
    }
    if (!mobile) {
      ctx.throw(400, '请输入要绑定的手机号码。');
    }
    if (!nationCode) {
      ctx.throw(400, '请选择国际区号。');
    }
    await db.SettingModel.checkMobile(nationCode, mobile);
    const otherPersonal = await db.UsersPersonalModel.findOne({
      nationCode,
  ...
  .post('/changeMobile', async (ctx, next) => {
    ...
    if (!userPersonal.mobile) {
      ctx.throw(400, '您暂未绑定手机号，请刷新');
    }
    const { apiFunction, sendMessage } = ctx.nkcModules;
    let smsCodeObj = {};
    if (operation === 'verifyOldMobile') {
      //-- 验证旧手机 --
      ...
      await db.SmsCodeModel.ensureSendPermission(smsCodeObj);
      smsCodeObj.code = apiFunction.random(6);
    } else if (operation === 'verifyNewMobile') {
      //-- 验证新手机 --
      const { nationCode, mobile } = body;
      if (!mobile) {
        ctx.throw(400, '新手机号不能为空');
      }
      if (
        userPersonal.mobile === mobile &&
        userPersonal.nationCode === nationCode
      ) {
        ctx.throw(400, '您已绑定该手机号，请更换后重试');
      }
      await db.SettingModel.checkMobile(nationCode, mobile);
      const sameUserPersonal = await db.UsersPersonalModel.findOne({
        mobile,
        nationCode,
      });
      if (sameUserPersonal) {
        ctx.throw(400, '该号码已被其他用户绑定，请更换后重试');
      }
      const type = 'bindMobile';
      const ip = ctx.address;
      smsCodeObj = {
