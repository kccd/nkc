...
router
  .post('/', OnlyOperation(Operations.digestComment), async (ctx, next) => {
    //评论加精，加精奖励科创币
    ...
    data.comment = (
      await db.CommentModel.getCommentsByCommentsId([comment._id])
    )[0];
  ...
  .del('/', OnlyOperation(Operations.unDigestComment), async (ctx, next) => {
    //评论取消加精，取消加精后扣除科创币
