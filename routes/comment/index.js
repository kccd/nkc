...
router
  .get('/', async (ctx, next) => {
    ...
    const { user } = data;
    //虎丘当前文章下已经发布的文章
    let comments = await db.CommentModel.find({ sid, source: 'comment' }).sort({
      toc: 1,
    });
    //获取当前文章下的当前用户为发表的评论草稿
    let comment = await db.CommentModel.findOne({
      uid: state.uid,
      source: 'comment',
    })
      .sort({ toc: -1 })
    ...
    let document;
    if (comment) {
      document = await db.DocumentModel.findOne({
        did: comment.did,
        type: 'beta',
      });
    }
    ...
    if (type === 'create' && !comment) {
      ...
      if (type === 'publish') {
        ...
        if (toColumn) {
          ...
          if (
            !toColumn.selectedMainCategoriesId ||
            toColumn.selectedMainCategoriesId.length === 0
          ) {
