...
listRouter
  .get('/', OnlyOperation(Operations.visitProblemList), async (ctx, next) => {
    ...
    const problems = await db.ProblemModel.find(match)
      .sort({ toc: -1 })
      .skip(paging.start)
  ...
  .get('/:_id', OnlyOperation(Operations.visitProblem), async (ctx, next) => {
  ...
  .put('/:_id', OnlyOperation(Operations.modifyProblem), async (ctx, next) => {
    const { params, data, db, body } = ctx;
    const { user } = data;
    const { _id } = params;
    const problem = await db.ProblemModel.findOnly({ _id });
    if (problem.resolved && user.uid != problem.restorerId) {
      return await next();
    }
    ...
    const problemsType = await db.ProblemsTypeModel.findOnly({
      name: typeName,
    });
  ...
  .del('/:_id', OnlyOperation(Operations.deleteProblem), async (ctx, next) => {
    const { params, db } = ctx;
    const { _id } = params;
    const problem = await db.ProblemModel.findOnly({ _id });
