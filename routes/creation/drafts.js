...
router
  .get('/', OnlyUser(), async (ctx, next) => {
    ...
    const drafts = await db.CreationDraftsModel.find(queryMap)
      .sort({ toc: -1 })
      .skip(paging.start)
      .limit(Number(paging.perpage));
  ...
  .post('/editor', OnlyUnbannedUser(), async (ctx, next) => {
    ...
    if (!['create', 'modify', 'save', 'autoSave'].includes(type))
      ctx.throw(400, `未知 ${type}类型`);
    let draft;
    if (type === 'create') {
      //创建草稿和文档
