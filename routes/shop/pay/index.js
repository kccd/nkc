...
router
  .use('/', OnlyUnbannedUser(), async (ctx, next) => {
    ...
    for (const orderId of ordersId) {
      ...
      if (!order) {
        ctx.throw(400, `未找到ID为【${orderId}】的订单`);
      }
      if (order.buyUid !== user.uid) {
        ctx.throw(403, `订单【${orderId}】错误`);
      }
      if (order.closeStatus) {
        ctx.throw(400, '订单已关闭');
      }
      if (order.orderStatus !== 'unCost') {
        ctx.throw(400, `订单【${orderId}】暂不需要付款，请勿重复提交`);
      }
      orders.push(order);
    }
    ...
    for (let order of data.orders) {
      for (let a = 0; a < order.params.length; a++) {
        ...
        if (product.productStatus === 'stopsale') {
          ctx.throw(
            400,
            `商品id为(${product.productId})的商品停售，不可购买，请重新下单`,
          );
        }
      }
    }
  ...
  .post('/', OnlyUnbannedUser(), async (ctx, next) => {
    ...
    if (userMainScore < totalMoney) {
      ctx.throw(
        400,
        `你的${mainScore.name}不足，请先充值或选择其他付款方式支付`,
      );
    }
    if (totalMoney !== totalPrice) {
      ctx.throw(400, '订单价格已被修改，请重新发起付款或刷新当前页面');
    }
