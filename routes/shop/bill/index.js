...
router
  .get('/', OnlyUnbannedUser(), async (ctx, next) => {
    ...
    if (!cartsId) {
      billType = 'product';
      if (!productCount) {
        ctx.throw(400, '请选择商品数量');
      }
      if (!paraId) {
        ctx.throw(400, '请选择商品');
      }
      const billProducts = await db.ShopProductsParamModel.findOne({
        _id: paraId,
      });
    ...
    for (const cart of data.cartsData) {
      ...
      productsObj[product.productId] = {
        product,
        freight: product.isFreePost
          ? ''
          : product.freightTemplates[freightId] || product.freightTemplates[0],
        params: [],
      };
      paramsObj[productParam._id] = {
        productParam,
        cartId: _id,
        price: productParam.useDiscount
          ? productParam.price
          : productParam.originPrice,
        count,
      };
    }
    for (const paramId in paramsObj) {
      if (!paramsObj.hasOwnProperty(paramId)) {
        continue;
      }
    ...
    for (const productId in productsObj) {
      if (!productsObj.hasOwnProperty(productId)) {
        continue;
      }
    ...
    for (const cart of data.cartsData) {
      ...
      if (cart.product.isFreePost) {
        ...
        if (!ffp && ffp !== 0) {
          ffp = cart.product.freightTemplates[0].firstPrice;
        }
        if (!afp && afp !== 0) {
          afp = cart.product.freightTemplates[0].addPrice;
        }
      ...
      if (cart.product.vipDiscount) {
        for (let v = 0; v < cart.product.vipDisGroup.length; v++) {
          if (
            data.user &&
            data.user.grade._id === cart.product.vipDisGroup[v].vipLevel + 1
          ) {
      ...
      cart.vipNum = vipNum;
      // 计算商品价格合计(含运费)
      if (!cart.productParam) {
        ctx.throw(400, '商品规格信息已更新，请重新选择');
      }
      let productPrice =
        cart.count * (cart.productParam.price * (vipNum / 100));
    ...
    for (let n in newCartData) {
      newCartData[n].carts = await db.ShopGoodsModel.checkOutPurchaseLimit(
        newCartData[n].carts,
        user.uid,
      );
    }
  ...
  .put('/add', OnlyUnbannedUser(), async (ctx, next) => {
    const { data, db, body } = ctx;
    const { user } = data;
    const { productParamId, cartId, count } = body;
    let cart = await db.ShopCartModel.findOne({ _id: cartId });
    // 检测商品库存
    let productParam = await db.ShopProductsParamModel.findOne({
      _id: productParamId,
    });
    ...
    if (
      product.purchaseLimitCount !== -1 &&
      count > product.purchaseLimitCount
    ) {
      ctx.throw(400, `该商品最多可购买 ${product.purchaseLimitCount} 件`);
    }
    // 计算小计价格(包括vip价格)
    let vipNum = 100;
    if (product.vipDiscount) {
      for (let v = 0; v < product.vipDisGroup.length; v++) {
        if (
          data.user &&
          data.user.authLevel == product.vipDisGroup[v].vipLevel
        ) {
    ...
    if (!product.isFreePost) {
      freightPrices =
        product.freightPrice.firstFreightPrice +
        product.freightPrice.addFreightPrice * (count - 1);
    }
  ...
  .put('/plus', OnlyUnbannedUser(), async (ctx, next) => {
    const { data, db, body } = ctx;
    const { user } = data;
    const { productParamId, cartId, count } = body;
    let cart = await db.ShopCartModel.findOne({ _id: cartId });
    ...
    let product = await db.ShopGoodsModel.findOne({
      productId: productParam.productId,
    });
    data.sellUid = product.uid;
    data.productId = product.productId;
