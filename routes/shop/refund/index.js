...
router
  .post('/', OnlyUnbannedUser(), async (ctx, next) => {
    ...
    let param;
    const orderDB = order;
    if (order.buyUid !== user.uid) {
      ctx.throw(400, '您没有权限操作别人的订单');
    }
    ...
    // 判断订单退款的状态
    if (refundStatus === 'ing') {
      ctx.throw(400, '订单正在退款中，请勿重复提交申请');
    }
    if (refundStatus === 'success') {
      ctx.throw(400, '订单退款已完成，请勿重复提交申请');
    }
    // 判断申请是否已存在
    const refunds = await db.ShopRefundModel.find({ orderId }).sort({
      toc: -1,
    });
    ...
    if (root && !refunds.length) {
      ctx.throw(400, '请先向卖家提出申请，卖家拒绝后可向平台提出申请');
    }
    if (!type) {
      if (orderStatus !== 'unCost') {
        ctx.throw(400, '请选择退款类型（退款、退款+退货）');
      }
    } else {
    ...
    if (!reason) {
      ctx.throw(400, '理由不能为空');
    }
    if (tools.checkString.contentLength(reason) > 1000) {
      ctx.throw(400, '理由不能超过1000个字节');
    }
    const time = Date.now();
    // 未付款时取消订单
    if (orderStatus === 'unCost') {
      ...
      if (refundMoney < 0) {
        ctx.throw(400, '退款金额不能小于0');
      }
      if (param) {
        if (refundMoney > param.singlePrice * param.count) {
          ctx.throw(400, '退款金额不能超过要退款的商品的金额');
        }
      } else {
        if (refundMoney > orderPrice) {
          ctx.throw(400, '退款金额不能超过全部商品的总金额');
        }
      }
      r.money = refundMoney;
      if (orderStatus === 'unShip') {
        ...
        if (type === 'money') {
          ...
          if (root) {
            ctx.throw(400, '请求平台介入时退款方式只能选择【只退款】');
          }
          r.status = 'B_APPLY_RP';
        }
      }
      ...
      if (param) {
        await db.ShopCostRecordModel.updateMany(
          { costId: param.costId },
          {
            $set: {
              refundStatus: 'ing',
        ...
        await db.ShopCostRecordModel.updateMany(
          { orderId: order.orderId, refundStatus: '' },
          {
            $set: {
              refundStatus: 'ing',
            },
  ...
  .use(
    '/:_id',
    singleRefundRouter.routes(),
    singleRefundRouter.allowedMethods(),
  );
