...
router
  ...
  .get('/', async (ctx, next) => {
    ...
    if (state.uid) {
      const lock = await nkcModules.redLock.redLock.lock(
        `dailyLogin:${state.uid}`,
        5000,
      );
      try {
        await ctx.db.KcbsRecordModel.insertSystemRecord(
          'dailyLogin',
          user,
          ctx,
        );
  ...
  .get('/', async (ctx, next) => {
    ...
    // 置顶文章轮播图
    data.ads = (
      await db.ThreadModel.getHomeRecommendThreads(fidOfCanGetThreads)
    ).movable;
    ...
    if (threadListType === 'reply') {
      ...
      for (let i = 0; i < users.length; i++) {
        const { uid } = users[i];
        users[i].avatar = nkcModules.tools.getUrl(
          'userAvatar',
          users[i].avatar,
        );
      ...
      // 最新页置顶文章
      data.latestToppedThreads = await db.ThreadModel.getLatestToppedThreads(
        fidOfCanGetThreads,
      );
      ...
      if (d === 'all' || d === 'column') {
        //获取关注的专栏id
        subColumnId = await db.SubscribeModel.getUserSubColumnsId(
          data.user.uid,
        );
        if (subColumnId.length) {
          ...
          //查找专栏文章引用
          const subColumnPosts = await db.ColumnPostModel.find(
            { columnId: { $in: subColumnId }, hidden: false },
            { pid: 1, columnId: 1 },
          );
      ...
      if (d === 'all' || d === 'thread') {
        subTid = await db.SubscribeModel.getUserSubThreadsId(
          data.user.uid,
          'sub',
        );
      }
      ...
      for (const post of posts) {
        ...
        if (user.uid !== null) {
          user.homeUrl = nkcModules.tools.getUrl('userHome', user.uid);
        }
        ...
        if (quote !== null) {
          if (quote.user.uid !== null) {
            quote.user.homeUrl = nkcModules.tools.getUrl(
              'userHome',
              quote.user.uid,
            );
          }
        ...
        if (subTid.includes(tid)) {
          ...
          a = {
            ...
            user: {
              ...
              avatar: forum.logo
                ? nkcModules.tools.getUrl('forumLogo', forum.logo)
                : null,
      ...
      if (data.user) {
        data.subColumns = await db.SubscribeModel.getUserSubColumns(
          data.user.uid,
        );
      }
    ...
    if (threadListType !== 'subscribe' && threadListType !== 'reply') {
      ...
      let sort = { tlm: -1 };
      if (s === 'toc' || threadListType === 'column') {
        sort = { toc: -1 };
      }
      ...
      if (threadListType === 'column') {
        ...
        let columnArticles = await db.ArticleModel.find({
          _id: { $in: aidArr },
        });
        ...
        for (const c of columnPosts) {
          ...
          if (c.type === threadType) {
            ...
            if (t) {
              ...
              t.type = 'newArticle';
              (t.document.content = nkcModules.nkcRender.htmlToPlain(
                t.document.content,
                200,
              )),
                //获取文章的专栏信息
    ...
    if (!state.isApp) {
      if (threadListType !== 'thread') {
        data.articleThreads = await db.ThreadModel.getLatestThreads(
          fidOfCanGetThreads,
        );
      }
