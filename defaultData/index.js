...
async function initForum() {
  ...
  if (fcDB === 0) {
    for (const f of forumCategories) {
      ...
      if (firstForumCategoryId === undefined) {
        firstForumCategoryId = _id;
      }
...
async function initDefaultHomeBlocks() {
  ...
  const defaultHomeBlocksDB = await db.HomeBlockModel.find(
    { defaultBlock: true },
    { _id: 1 },
  );
  ...
  for (const block of defaultHomeBlocks) {
    ...
    if (defaultHomeBlocksIdDB.includes(_id)) {
      continue;
    }
...
async function initAccessControl() {
  ...
  for (const source of sources) {
    if (existSources.includes(source)) {
      continue;
    }
    const platformData = {
      enabled: false,
